...
// 		}
	...
	public function save($quiz = null) {
		...
		/**
		 * Save statistics points in ld_aq_points column also, to be used when negative marking is enabled
		 */
		ld_aq_save_statistics_points($statisticRefMapper_id, $values);
	...
	private function makeDataList($quiz, $array, $userId, $modus) {
		...
		foreach($question as $q) {
             ...
             * Comments by Webxity:
             *
             * This logic does not allow to support `points` value to be negative value.
             * It return false if negative `points` exists.
             * Therefore disabled this logic to support negative `points` value.
             *
             */
		...
		foreach($array as $k => $v) {
			...
			$s->setQuestionTime( isset( $v['time'] ) ? $v['time'] : $avgTime);
	...
	public static function ajaxLoadStatisticUser($data, $func) {
		...
		foreach($statisticUsers as $statistic) {
			...
			/**
			 * Override points and use points from ld_aq_points column, if LearnDash Retake Quiz negative marking is enabled
			 */
			$points = ld_aq_get_statistics_points($refId, $statistic->getQuestionId());
