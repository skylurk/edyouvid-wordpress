		...
		if ( ! empty( $editor_context->post ) ) {
			array_push(
				...
				array(
					...
					'title' => __( 'Reports for LearnDash', 'learndash-reports-by-wisdmlabs' ),
					'icon'  => null,
				)
			);
		}
...
if ( ! function_exists( 'wrld_register_pattern_category' ) ) {
	...
	function wrld_register_pattern_category() {
		register_block_pattern_category(
			'wisdm-ld-reports',
			array( 'label' => __( 'Reports for Learndash', 'learndash-reports-by-wisdmlabs' ) )
		);
	}
}
...
if ( ! function_exists( 'wrld_db_install' ) ) {
	...
	function wrld_db_install() {
		...
		add_option( 'wrld_db_version', $wrld_db_version );
	}
}
...
if ( ! function_exists( 'wrld_activation_notices' ) ) {
	...
	function wrld_activation_notices() {
		echo "<div class='error'>
			<p>LearnDash LMS plugin is not active. In order to make <strong>Reports for LearnDash </strong> plugin work, you need to install and activate LearnDash LMS first.</p>
		</div>";
	}
}
...
if ( ! function_exists( 'wrld_free_onboarding_modal' ) ) {
	...
	function wrld_free_onboarding_modal() {
		...
		if ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) {
			...
			wp_enqueue_script( 'wrld_modal_script', WRLD_REPORTS_SITE_URL . '/assets/js/wrld-modal.js', array( 'jquery' ), WRLD_PLUGIN_VERSION, true );
			wp_localize_script( 'wrld_modal_script', 'wrld_modal_script_object', array( 'wp_ajax_url' => admin_url( 'admin-ajax.php' ) ) );
			$modal_head              = __( 'Welcome to LearnDash LMS - Reports!', 'learndash-reports-by-wisdmlabs' );
...
if ( ! function_exists( 'wrld_free_upgrade_to_pro_modal' ) ) {
	...
	function wrld_free_upgrade_to_pro_modal() {
		...
		if ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) {
			...
			wp_localize_script( 'wrld_modal_script', 'wrld_modal_script_object', array( 'wp_ajax_url' => admin_url( 'admin-ajax.php' ) ) );
			$modal_head              = __( 'Welcome to LearnDash LMS - Reports!', 'learndash-reports-by-wisdmlabs' );
			$modal_description       = __( 'The plugin has been updated successfully. You are a few steps away from launching the Reports Dashboard. The next step is to update the LearnDash LMS - Reports PRO for LearnDash plugin on the “All Plugins” Page of your WordPress dashboard', 'learndash-reports-by-wisdmlabs' );
			$info_url                = '#';
...
if ( ! function_exists( 'wrld_add_review_notice' ) ) {
	...
	function wrld_add_review_notice() {
		...
		$message_head = __( 'You have been using Reports for LearnDash for over a week now!', 'learndash-reports-by-wisdmlabs' );
		$message      = __( 'Helping other users is our motivation and if you could give us a 5 star rating on WordPress, that would help other users make an informed decision while choosing Reports and it would mean the world to us!', 'learndash-reports-by-wisdmlabs' );
...
if ( ! function_exists( 'wrld_add_upgrade_notice' ) ) {
	...
	function wrld_add_upgrade_notice() {
		...
		if ( isset( $_GET['ld-reports-upgrade-skip'] ) && true == sanitize_text_field( wp_unslash( $_GET['ld-reports-upgrade-skip'] ) ) ) {
			...
			if ( ! check_admin_referer( 'ld-reports-upgrade-skip' ) ) {
					return;
			}
		...
		$nonce             = wp_create_nonce( 'ld-reports-upgrade-skip' );
...
if ( ! function_exists( 'wrld_add_recomendation_notice' ) ) {
	...
	function wrld_add_recomendation_notice() {
		if ( current_user_can( 'manage_options' ) && defined( 'LDRP_PLUGIN_VERSION' ) && version_compare( LDRP_PLUGIN_VERSION, WRLD_RECOMENDED_LDRP_PLUGIN_VERSION, '<' ) ) {
			wp_enqueue_script( 'ldrp_update_script', WRLD_REPORTS_SITE_URL . '/assets/js/wrld-update.js', array( 'jquery', 'updates' ), LDRP_PLUGIN_VERSION, true );
			...
			if ( 'valid' !== $validity ) {
				...
				$cta_message_part_2 = __( ' and then proceed to update the LearnDash LMS - Reports PRO plugin.', 'learndash-reports-by-wisdmlabs' );
			...
			<div class='notice-error notice is-dismissible'>
				<div style='display:inline-flex; justify-content:center; align-items:center;' class='wrld-plugin-dependency-installation'>
					<p>
						<strong>
						<?php echo wp_kses_post( __( 'Learndash recommends you to update the LearnDash LMS - Reports PRO for LearnDash to its latest version now.', 'learndash-reports-by-wisdmlabs' ) ); ?>
						</strong>
						<?php
...
if ( ! function_exists( 'wrld_show_review_notice' ) ) {
	...
	function wrld_show_review_notice( $message_head, $message, $button_text, $link = 'https://wordpress.org/support/plugin/wisdm-reports-for-learndash/reviews/#new-post' ) {
		...
		wp_enqueue_script( 'wrld_notice_script', WRLD_REPORTS_SITE_URL . '/assets/js/wrld-notices.js', array( 'jquery' ), WRLD_PLUGIN_VERSION, true );
...
if ( ! function_exists( 'wrld_get_common_script_localized_data' ) ) {
	...
	function wrld_get_common_script_localized_data() {
		...
		$data = array(
			...
			'upgrade_link'                      => 'https://www.learndash.com/reports-by-learndash',
			...
			'notice_content'                    => array(
												...
												'li_3'   => __( 'Once launched, only the admin can access this page. To provide access to others, navigate to the WordPress dashboard > LearnDash Reports > Settings tab.', 'learndash-reports-by-wisdmlabs' ),
											),
...
if ( ! function_exists( 'wrld_register_admin_ajax_callbacks' ) ) {
	...
	function wrld_register_admin_ajax_callbacks() {
		...
		// TODO: We may want to refactor the following block of code as it runs on every admin page load.
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Data is not used for action or DB query.
	...
	function wisdm_reports_free_multi_installation_check() {
		...
		foreach ( $installed_plugins as $key => $plugin_data ) {
			if ( in_array( $plugin_data['TextDomain'], $free_plugin_uris, true ) ) {
				if ( isset( $plugin_data['UpdateURI'] ) && '' === $plugin_data['UpdateURI'] ) {
