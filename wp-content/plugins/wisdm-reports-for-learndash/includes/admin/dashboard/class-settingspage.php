...
if ( ! class_exists( 'SettingsPage' ) ) {
	...
	class SettingsPage {
		...
		public static function render() {
			...
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Data is not used for action or DB query.
			$subtab = isset( $_GET['subtab'] )
				// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Data is not used for action or DB query.
				? sanitize_text_field( wp_unslash( $_GET['subtab'] ) )
				: null;

			if ( isset( $subtab ) && 'data-upgrade' === $subtab ) {
			...
			<div class='wrld-sidebar-block'>
					...
					<div class='wrld-sidebar-body'>
						<div class='wrld-sidebar-body-section'>
							...
							<ul>
							<li><a href="https://www.learndash.com/support/docs/add-ons/reports-for-learndash/" target='__blank'><?php esc_html_e( 'Documentation', 'learndash-reports-by-wisdmlabs' ); ?></a></li>
							</ul>
						</div>
					</div>
				</div>
		...
		public static function get_add_menu_link_setting( $settings_data ) {
			$add_menu_link         = isset( $settings_data['wrld-menu-config-setting'] ) && ( 'true' === strval( $settings_data['wrld-menu-config-setting'] ) ) ? true : false;
			$add_menu_student_link = isset( $settings_data['wrld-menu-student-setting'] ) && ( 'true' === strval( $settings_data['wrld-menu-student-setting'] ) ) ? true : false;
		...
		public static function get_accessibility_setting( $settings_data ) {
			...
			$gl_access              = isset( $settings_data['dashboard-access-roles'] ) && ( 'true' === strval( $settings_data['dashboard-access-roles']['group_leader'] ) ) ? true : false;
			$wdm_instructor_access  = isset( $settings_data['dashboard-access-roles'] ) && ( 'true' === strval( $settings_data['dashboard-access-roles']['wdm_instructor'] ) ) ? true : false;
				...
				<div class='wrld-accessibility-setting-section'>
					...
					<div>
						<label for='wrld-menu-access-setting-wdm-instructor' class="checkbox-label <?php echo esc_attr( $wdm_instructor_access ? '' : 'no-access' ); ?>">
							...
							<span class="<?php echo esc_attr( $instructor_role_status ); ?>"><?php esc_html_e( 'Instructor role by LearnDash', 'learndash-reports-by-wisdmlabs' ); ?></span>
							...
							if ( 'disabled' === $instructor_role_status ) {
								?>
										<span><a href="https://www.learndash.com/reports-by-learndash" target='_blank'><?php esc_html_e( 'Learn more', 'learndash-reports-by-wisdmlabs' ); ?></a></span>
									<?php
							}
							?>
						</label>
				...
				<div class="exclude-field users-field <?php echo defined( 'LDRP_PLUGIN_VERSION' ) ? 'pro-active' : 'pro-inactive'; ?>">
					...
					<div class="right-fields">
						...
						<?php if ( ! defined( 'LDRP_PLUGIN_VERSION' ) ) : ?>
							<a class="wrld-upgrade-button" href="https://www.learndash.com/reports-by-learndash" target='__blank'>
								<button class='wrld-sidebar-body-button'><?php esc_html_e( 'Upgrade to PRO', 'learndash-reports-by-wisdmlabs' ); ?></button>
							</a>
						<?php endif; ?>
					</div>
		...
		public static function get_time_tracking_setting( $settings_data ) {
			...
			global $wpdb;
				...
				$key   = 'time_entries_count';
				$group = 'learndash_reports_free';
				$count = wp_cache_get( $key, $group, false, $found );

				if ( ! $found ) {
					$table_name = $wpdb->prefix . 'ld_time_entries';

					// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Necessary to retrieve data from custom table.
					$count = $wpdb->get_var(
						$wpdb->prepare(
							// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- %i placeholder is not available in WordPress < 6.2.
				...
				if ( 0 != $count && ( ! get_option( 'migrated_course_time_access_data', false ) ) ) {
				?>
			<div class="wrld-upgrade-banner">
				...
				<div style="margin-top: 24px;">
					...
					<?php else : ?>
						...
						<a class="wrld-upgrade-button" href="https://www.learndash.com/reports-by-learndash" target='__blank' style="margin-top: 20px; margin-bottom: 35px;">
							<button class='wrld-sidebar-body-button'><?php esc_html_e( 'Upgrade to PRO', 'learndash-reports-by-wisdmlabs' ); ?></button>
						</a>
						<div class="time_frontend_popup_image"><img src="<?php echo esc_url( WRLD_REPORTS_SITE_URL . '/includes/admin/dashboard/assets/images/time_tracking_popup.png' ); ?>" alt=""></div>
		...
		public static function get_data_upgrade_setting( $settings_data ) {
			...
			<form method="post" class="address-step">
				...
				<p class="reports-setup"><strong><?php esc_html_e( 'Enhance the loading speed and performance of LearnDash LMS - Reports.', 'learndash-reports-by-wisdmlabs' ); ?></strong></p>
				<p class="wrld-dashboard-text">
					<?php
					printf(
						// Translators: %1$s: HTML opening tag, %2$s: HTML closing tag, %3$s: HTML opening tag, %4$s: HTML closing tag.
						esc_html__( ' We request you to perform the following actions to %1$s upgrade learners and group data in the database %2$s. By Incorporating the LearnDash user enrollment data in a better format into the  course and group  post meta tables, you will %3$s experience substantial improvements in report loading times and overall performance. %4$s', 'learndash-reports-by-wisdmlabs' ),
						'<strong>',
						'</strong>',
						'<strong>',
						'</strong>'
				...
				<p>
					<span class="font-red">
						...
						printf(
							...
							esc_html__( 'Note: We highly recommend you to perform the following action %1$s if your site contains more than 3000 users. %2$s', 'learndash-reports-by-wisdmlabs' ),
							'<u>',
							'</u>'
						);
						?>
							...
							if ( $course_updated_on ) {
								...
								<div class="last-updated-on">
									...
									printf(
										...
										esc_html__('Last run: %s', 'learndash-reports-by-wisdmlabs'),
										'<strong>' . esc_html( date_i18n( 'Y-m-d H:i:s', $course_updated_on ) ) . '</strong>'
									);
									?>
								</div>
								<?php
							...
							if ( $group_updated_on ) {
								...
								<div class="last-updated-on">
									...
									printf(
										// Translators: %s: Date and time.
										esc_html__( 'Last run: %s', 'learndash-reports-by-wisdmlabs' ),
										'<strong>' . esc_html( date_i18n( 'Y-m-d H:i:s', $group_updated_on ) ) . '</strong>'
									);
									?>
								</div>
								<?php
				...
				<p class="wrld-setup-actions step">
					<button class="button-primary button button-large">
						<a href="<?php echo esc_url( admin_url( 'admin.php?page=wrld-dashboard-page' ) ); ?>">
							<?php esc_html_e( 'Continue', 'learndash-reports-by-wisdmlabs' ); ?>
						</a>
					</button>
				</p>
				<?php } ?>
