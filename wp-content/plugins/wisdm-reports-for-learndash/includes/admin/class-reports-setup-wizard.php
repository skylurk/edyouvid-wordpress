...
class Reports_Setup_Wizard {
	...
	public function enqueue_scripts() {
		if ( is_rtl() ) {
			wp_enqueue_style( 'reports_setup_css', WRLD_REPORTS_SITE_URL . '/includes/admin/dashboard/assets/css/wizard.rtl.css', [], WRLD_PLUGIN_VERSION );
		} else {
			wp_enqueue_style( 'reports_setup_css', WRLD_REPORTS_SITE_URL . '/includes/admin/dashboard/assets/css/wizard.css', [], WRLD_PLUGIN_VERSION );
		}
	...
	public function setup_wizard() {
		...
		if ( empty( $_GET['page'] ) || 'wrld-setup' !== $_GET['page'] ) {
			return;
		}
		$default_steps = array(
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Data is not used for authorized action or DB query.
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			<title><?php esc_html_e( 'Reports For LearnDash &rsaquo; Setup Wizard', 'learndash-reports-by-wisdmlabs' ); ?></title>
		...
		<h1 class="wc-logo"><a href="https://www.learndash.com/" target="_blank"><img src="<?php echo esc_url( WRLD_REPORTS_SITE_URL ); ?>/assets/images/learndash.png" alt="<?php esc_attr_e( 'Reports', 'learndash-reports-by-wisdmlabs' ); ?>" /></a></h1>
		<?php
	}
	...
	public function wc_setup_ready() {
		...
		$docs_url   = 'https://www.learndash.com/support/docs/add-ons/reports-for-learndash/';
		$wrld_page  = get_option( 'ldrp_reporting_page', false );
		$help_text  = sprintf(
			/* translators: %1$s: link to docs */
			__( 'Visit learndash.com to learn more about <a href="%1$s" target="_blank">getting started</a>.', 'learndash-reports-by-wisdmlabs' ),
			$docs_url
		);
		?>
...
if ( ! get_option('wrld-setup') && version_compare(WRLD_PLUGIN_VERSION, '1.8.1', '<=') ) {
