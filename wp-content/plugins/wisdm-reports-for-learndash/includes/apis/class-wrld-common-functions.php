	...
	class WRLD_Common_Functions {
		...
		public static function get_list_of_users_enrolled_in_courses( $course_ids = array() ) {
			...
			if ( ! empty( $course_ids ) ) {
				foreach ( $course_ids as $course_id ) {
					...
					// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verification is done in WP REST API handler.
		...
		public static function wrld_dropdown_users( $args = '' ) {
			$defaults = array(
				...
				'order'                   => 'ASC',
				'include'                 => '',
				// phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_exclude -- Empty default parameter.
				'exclude'                 => '',
				'multi'                   => 0,
				'show'                    => 'display_name',
				'echo'                    => 1,
			...
			if ( false === $data || ( defined( 'WRLD_DISABLE_TRANSIENTS' ) && WRLD_DISABLE_TRANSIENTS ) || ( isset( $_GET['disable_cache'] ) && ! empty( $_GET['disable_cache'] ) ) ) {
				...
				if ( $data ) {
					...
					if ( ! empty( $data['exclude_courses'] ) ) {
						// phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_post__not_in -- Optional query parameter.
