...
class WRLD_Quiz_Reporting_Tools extends WRLD_Common_Functions {
	...
	public function wrld_get_question_details() {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Used in public REST endpoint handler.
		$question_data = wp_unslash( $_REQUEST['question_data'] );
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Used in public REST endpoint handler.
		$question_no = sanitize_text_field( wp_unslash( $_REQUEST['q_no'] ) );
		$question    = json_decode( $question_data );
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Used in public REST endpoint handler.
	...
	public function wrld_get_student_dashboard_results() {
		...
		if ( ! empty( $data ) ) {
			foreach ( $data as &$statistic ) {
				$extradot                   = strlen( substr( wp_strip_all_tags( get_the_title( $statistic['quiz_post_id'] ) ), 0, 100 ) ) == 100 ? ' ...' : '';
				$statistic['quiz_title']    = "<a target='_blank' style='display:flex;text-decoration: none;' href='" . add_query_arg(
					...
					''
				) . "'>" . substr( wp_strip_all_tags( get_the_title( $statistic['quiz_post_id'] ) ), 0, 100 ) . $extradot . ' <span style="margin-left:3px;font-size:15px;text-decoration:none;"  class="dashicons dashicons-external"></span></a>';
	...
	public function qre_live_search_results() {
		// Get Inputs.
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Public REST endpoint handler.
		$search_term = sanitize_text_field( ! empty( $_GET['search_term'] ) ? wp_unslash( $_GET['search_term'] ) : '' );// WPCS: sanitization ok, CSRF ok.
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Public REST API handler.
		do_action( 'wpml_switch_language', sanitize_text_field( wp_unslash( $_GET['wpml_lang'] ) ) ); // switch the content language
		$results        = array();
		$user_results   = array();
	...
	public static function qre_search_courses( $query = '', $skip_search = false ) {
		...
		if ( ! empty( $excluded_courses ) ) {
			// phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_exclude -- Optional parameter.
	...
	public static function qre_search_users( $search_string = '', $skip_search = false ) {
		...
		if ( false === $skip_search ) {
			$args = array(
				...
				// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- Required meta query.
	...
	public static function check_if_user_ids_actually_present( $user_ids ) {
			...
			$user_ids_present = $wpdb->get_col(
				$wpdb->prepare(
					'SELECT ID FROM ' . $wpdb->prefix . 'users WHERE ID IN (%s)',
					$user_ids
				)
			);
