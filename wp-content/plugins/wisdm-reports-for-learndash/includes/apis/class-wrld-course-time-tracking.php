...
class WRLD_Course_Time_Tracking extends WRLD_Common_Functions {
	...
	public function createTimeSpentEntryForUser( $course_id, $user_id, $group_id = null ) {
		...
		$table_name_time_spent = $wpdb->prefix . 'ld_course_time_spent';
		$query                 = $wpdb->prepare(
			// phpcs:ignore	WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Safe custom table variable.
			"SELECT COUNT(*) FROM {$table_name_time_spent} WHERE course_id = %d AND user_id = %d",
			$course_id,
			$user_id
		);

		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Custom prepared query.
		...
		if ( $entry_exists ) {
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Custom query.
			$wpdb->insert(
				$table_name_time_spent,
				$data,
				$data_formats
	...
	public function get_course_time_spent() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verification is done automatically in WP REST API handler.
		do_action( 'wpml_switch_language', sanitize_text_field( wp_unslash( $_GET['wpml_lang'] ) ) ); // switch the content language
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Custom query and up-to-date data.
		...
		if ( empty( $time_spent_row ) || ( defined( 'WRLD_DISABLE_TRANSIENTS' ) && WRLD_DISABLE_TRANSIENTS ) || $time_spent_row->expires_on <= current_time( 'timestamp' ) || $request_data['disable_cache'] ) {
			...
			if ( isset( $request_data['learner'] ) && ! empty( $request_data['learner'] ) ) {
				...
				if ( 'all' !== strval( $request_data['duration'] ) ) {
					...
					$where           = $where . "AND ct.enrollment_date > '" . esc_sql( $enrollment_date ) . "' ";
					$where1          = $where . "AND enrollment_date > '" . esc_sql( $enrollment_date ) . "' ";
				}
				...
				// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Variable is already prepared.
				$query   = $wpdb->prepare(
			...
			";
				...
				if ( ! is_null( $accessible_users ) && -1 !== intval( $accessible_users ) ) {
					$user_ids_str = implode(
						',',
						array_map(
							function ( $user_id ) {
								return "'" . esc_sql( $user_id ) . "'";
							},
							$accessible_users
						)
					);
				...
				if ( 'all' !== strval( $request_data['duration'] ) ) {
					$afterDuration   = $request_data['duration'];
					$enrollment_date = strtotime( gmdate( 'Y-m-d', strtotime( '-' . $afterDuration ) ) );
					$where           = $where . "AND enrollment_date > '" . esc_sql( $enrollment_date ) . "' ";
				}

				$course_id = intval( $request_data['course'] );
				$duration  = intval( $request_data['timeperiod'] );

				$checkCourseUserssql = $wpdb->prepare(
					"SELECT COUNT(DISTINCT user_id) AS total_users
						FROM {$wpdb->prefix}ld_course_time_spent
						WHERE course_id = %d
						$where", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Variable is already prepared.
					$course_id
				);
				...
				$msg_no_data = ( 'false' == $request_data['rtype'] || false == $request_data['rtype'] ) ? __( 'There are no learners yet who are in progress or have completed the selected course” or select a different course.', 'learndash-reports-by-wisdmlabs' ) : __( 'All the learners have completed the selected course. To view time spent data either toggle to “Courses completed” or select a different course.', 'learndash-reports-by-wisdmlabs' );
				...
				$totalUsersql = $wpdb->prepare(
					"SELECT COUNT(DISTINCT user_id) AS total_users FROM {$wpdb->prefix}ld_course_time_spent WHERE course_id = %d",
					$course_id
				);
				...
				$query_average = $wpdb->prepare(
					"SELECT AVG(total_time_spent) AS average_time_spent
						FROM {$wpdb->prefix}ld_course_time_spent
						WHERE course_id = %d {$where};", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Variable is already prepared.
					$course_id
				);
				...
				// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Variables are already prepared.
				$sql = $wpdb->prepare(
					"
	...
	public function get_course_time_details() {
		...
		if ( 0 === $time_spent_lower_limit && 0 === $time_spent_upper_limit ) {
			...
			if ( $time_spent_lower_limit > 0 && 0 === $time_spent_upper_limit ) {
				$timeCondition = "total_time_spent >= '" . esc_sql( $time_spent_lower_limit ) . "' ";
			} else {
				$timeCondition = "total_time_spent >= '" . esc_sql( $time_spent_lower_limit ) . "' AND total_time_spent < '" . esc_sql( $time_spent_upper_limit ) . "' ";
			}
		}
		...
		$sql = $wpdb->prepare(
			"SELECT ct.total_time_spent , ct.enrollment_date , u.display_name
				FROM {$table_course_time_spent} as ct , {$table_users} as u WHERE $timeCondition AND ct.user_id = u.id AND ct.course_id = %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Variables are already prepared.
			$course_id
		);
		...
		$sql2      = $wpdb->prepare(
			"SELECT * FROM {$table_course_time_spent} ct WHERE {$timeCondition} AND course_id = %d {$where} LIMIT {$items_per_page} OFFSET {$newoffset}", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Variables are already prepared and safe.
			$course_id
		);
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Custom prepared query.
	...
	public function get_course_time_spent_csv() {
		...
		";
			...
			if ( ! file_exists( $upload_dir['basedir'] . '/learndash_reports' ) ) {
				wp_mkdir_p( $upload_dir['basedir'] . '/learndash_reports' );
			}
			// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- Required to use fputcsv.
			...
			if ( $file ) {
				...
				// }
				// Closes the Csv file.
				// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose -- Required to use fputcsv.
				fclose( $file );
			} else {
			...
			$checkCourseUserssql = $wpdb->prepare(
				...
				FROM {$wpdb->prefix}ld_course_time_spent
				WHERE course_id = %d
				$where", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Variable is safe.
				$course_id
			);
			...
			if ( $file ) {
				...
				fputcsv( $file, array() );
				$headerData = ( 'false' == $request_data['rtype'] || false == $request_data['rtype'] ) ? array( 'Learners completed in below ranges:' ) : array( __( 'Learners Progress in below ranges:', 'learndash-reports-by-wisdmlabs' ) );
	...
	public function get_quiz_completion_time() {
		...
		if ( empty( $time_spent_row ) || ( defined( 'WRLD_DISABLE_TRANSIENTS' ) && WRLD_DISABLE_TRANSIENTS ) || $time_spent_row->expires_on <= current_time( 'timestamp' ) || $request_data['disable_cache'] ) {
			...
			if ( ! empty( $category_query ) ) {
                // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query -- Optional category tax query.
