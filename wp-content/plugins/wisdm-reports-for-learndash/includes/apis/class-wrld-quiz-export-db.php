...
if ( ! class_exists( 'WRLD_Quiz_Export_Db' ) ) {
	...
	class WRLD_Quiz_Export_Db {
		...
		public function delete_user_data_handling( $deleted_user_id ) {
			...
			$course_enrollments = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT * FROM {$wpdb->prefix}postmeta WHERE meta_key = 'wrld_course_users' AND meta_value LIKE %s",
					'%' . $wpdb->esc_like( $deleted_user_id ) . '%'
				),
				ARRAY_A
			);
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Custom query and up-to-date data.
			$group_enrollments = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT * FROM {$wpdb->prefix}wrld_group_users AND meta_value LIKE %s",
					'%' . $wpdb->esc_like( $deleted_user_id ) . '%'
				),
				ARRAY_A
			);
			...
			*/
			$query = $wpdb->prepare(
				...
				option_name LIKE %s OR 
				option_name LIKE %s OR",
				'%' . $wpdb->esc_like( 'wrld_course_learners_data_' ) . '%',
				'%' . $wpdb->esc_like( 'wrld_accessible_users_data_' ) . '%',
				'%' . $wpdb->esc_like( 'wrld_group_students_data_' ) . '%',
				'%' . $wpdb->esc_like( 'wrld_course_students_data_' ) . '%',
				'%' . $wpdb->esc_like( 'wrld_course_groups_students_data_' ) . '%'
			);
		...
		public static function get_learner_activity_log_count( $start_date, $end_date, $accessible_courses = null, $accessible_users = null, $excluded_users = array() ) {
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Custom query and up-to-date data.
			$result = $wpdb->get_results( $query, ARRAY_A );
			return count( $result );
		}
		...
		public static function get_inactive_users_info( $start_date, $end_date, $accessible_courses = null, $accessible_users = null, $excluded_users = array(), $page = 1, $limit = 10 ) {
			...
			if ( ! empty( $accessible_courses ) && -1 !== intval( $accessible_courses ) ) {
				$accessible_courses = array_map(
					function ( $course_id ) {
						return '"' . esc_sql( $course_id ) . '"';
					},
					$accessible_courses
				);
				$condition         .= sprintf( ' AND activity.course_id IN (%s)', implode( ',', $accessible_courses ) );
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Custom prepared query.
			$users = $wpdb->get_results( $sub_query );
			...
			// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Variable value is already escapaed.
			$query1 = $wpdb->prepare(
			...
			foreach ( $result as $user ) {
				$query2 = $wpdb->prepare(
					"SELECT * FROM {$wpdb->prefix}ld_time_entries WHERE user_id = %d AND activity_updated > %d ORDER BY activity_updated LIMIT 1;",
					$user['user_id'],
					$user['great']
				);
		...
		public static function get_inactive_users_info_count( $start_date, $end_date, $accessible_courses = null, $accessible_users = null, $excluded_users = array() ) {
			...
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Variable value is already escaped.
			$sub_query = $wpdb->prepare( "( SELECT user_id FROM {$wpdb->prefix}learndash_user_activity activity WHERE ( ( activity.activity_completed >= %d AND activity.activity_completed <= %d ) OR ( activity.activity_updated >= %d AND activity.activity_updated <= %d ) OR ( activity.activity_started >= %d AND activity.activity_started <= %d ) ) AND {$condition} ORDER BY activity.activity_updated DESC, activity.activity_completed DESC, activity.activity_started DESC ) UNION ( SELECT user_id FROM {$wpdb->prefix}ld_time_entries activity WHERE  ( activity.activity_updated >= %d AND activity.activity_updated <= %d ) AND {$condition} ORDER BY activity.activity_updated DESC ) ORDER BY user_id DESC;", $start_date, $end_date, $start_date, $end_date, $start_date, $end_date, $start_date, $end_date );//phpcs:ignore
			...
			// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Variable value is already escaped.
			$query1 = $wpdb->prepare(
				"SELECT * FROM (SELECT activity.user_id, 
				activity.post_id, 
				activity.course_id, 
		...
		public function get_statistic_ref_id( $user_id, $pro_quiz_id, $time ) {
			...
			$query = $db_instance->prepare(
				"SELECT statistic_ref_id FROM {$table_name} where user_id = %d AND quiz_id = %d AND create_time = %d ORDER BY quiz_id DESC",
				$user_id,
				$pro_quiz_id,
				$time
			);
