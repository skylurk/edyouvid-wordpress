...
class WRLD_Course_Progress_Info extends WRLD_Common_Functions {
	...
	public function get_learner_activity_log() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verification is done automatically in rest_cookie_check_errors().
		...
		if ( ! is_null( $accessible_users ) && -1 !== intval( $accessible_users ) && empty( $accessible_users ) ) {
			return new WP_Error(
				...
				array( 'requestData' => self::get_values_for_request_params( $request_data ), 'status' => 401 )
			);
		}
	...
	public function get_quiz_completion_rate() {
		...
		if ( empty( $time_spent_row ) || ( defined( 'WRLD_DISABLE_TRANSIENTS' ) && WRLD_DISABLE_TRANSIENTS ) || $time_spent_row->expires_on <= current_time( 'timestamp' ) || $request_data['disable_cache'] ) {
			...
			if ( ! empty( $category_query ) ) {
				// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query -- Optional category query.
	...
	public function get_quiz_passing_rate() {
		...
		if ( isset( $request_data['group'] ) && ! empty( $request_data['group'] ) ) {
			// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- Optional meta query.
		...
		if ( $course_count <= 0 ) {
			return new WP_Error(
				...
				array( 'requestData' => self::get_values_for_request_params( $request_data ), 'status' => 404 )
			);
		} else {
	...
	public function get_modulewise_activity( $course, $module, $start_date, $end_date, $accessible_users = null ) {
		...
		$quizzes = array_map( function( $quiz_id ) {
			return '"' . esc_sql( $quiz_id ) . '"';
		}, $quizzes );
		...
		$where         .= ' AND statref.create_time>="' . esc_sql( $start_date ) . '"';
		$where         .= ' AND statref.create_time<="' . esc_sql( $end_date ) . '"';
		$where         .= ' AND statref.course_post_id="' . esc_sql( $course ) . '"';
		$excluded_users = get_option( 'exclude_users', array() );
		if ( empty( $excluded_users ) || ! defined( 'LDRP_PLUGIN_VERSION' ) ) {
			$excluded_users = array();
		}
		if ( ! is_null( $accessible_users ) && -1 !== intval( $accessible_users ) ) {
			$accessible_users = array_diff( $accessible_users, $excluded_users );
			$accessible_users = array_map( function( $user_id ) {
		...
		$quiz_statistics_ref_table = esc_sql( LDLMS_DB::get_table_name( 'quiz_statistic_ref' ) );
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Custom query and up-to-date data.
		$attempts = $wpdb->get_results(
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared -- Safe custom table name variable and query, the query is already prepared.
			'SELECT * FROM ' . $quiz_statistics_ref_table . ' as statref WHERE ' . $where . $orderby . $limit
		);
	...
	public function get_modulewise_statistics( $course, $module, $start_date, $end_date ) {
		...
		if ( ( false !== $start_date ) && ( false !== $end_date ) ) {
			...
			$results = $wpdb->get_results(
				// phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber -- $placeholder_args is an array of replacement parameters.
				$wpdb->prepare(
					'SELECT COUNT(*) FROM ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_statistic_ref' ) ) . ' as statref WHERE statref.quiz_id IN (%s) AND statref.create_time >= %s AND statref.create_time <= %s ORDER BY %s %s LIMIT %d, %d',
                    $placeholder_args
				),
				ARRAY_A
			);
			...
			$results = $wpdb->get_results(
				// phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber -- $placeholder_args is an array of replacement parameters.
				$wpdb->prepare(
					'SELECT COUNT(*) FROM ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_statistic_ref' ) ) . ' as statref WHERE  statref.quiz_id IN (%s) AND statref.create_time >= %s ORDER BY %s %s LIMIT %d, %d',
					$placeholder_args
				),
				ARRAY_A
			);
			...
			$results = $wpdb->get_results(
                // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber -- $placeholder_args is an array of replacement parameters.
                $wpdb->prepare(
                    'SELECT COUNT(*) FROM ' . esc_sql( LDLMS_DB::get_table_name( 'quiz_statistic_ref' ) ) . ' as statref WHERE  statref.quiz_id IN (%s) ORDER BY %s %s LIMIT %d, %d',
                    $placeholder_args
                ),
                ARRAY_A
            );
	...
	public function get_course_list_info() {
		...
		if ( empty( $time_spent_row ) || ( defined( 'WRLD_DISABLE_TRANSIENTS' ) && WRLD_DISABLE_TRANSIENTS ) || $time_spent_row->expires_on <= current_time( 'timestamp' ) || $request_data['disable_cache'] ) {
			...
			if ( isset( $request_data['learner'] ) && ! empty( $request_data['learner'] ) ) {
				...
				foreach ( $courses as $course_id ) {
					...
					if ( ! empty( $quizzes ) ) {
						...
						$where        .= ' AND statref.create_time>="' . esc_sql( $start_date ) . '"';
						$where        .= ' AND statref.create_time<="' . esc_sql( $end_date ) . '"';
						$where        .= ' AND statref.course_post_id="' . esc_sql( $course_id ) . '"';
						$where        .= ' AND statref.user_id="' . esc_sql( $request_data['learner'] ) . '"';
						$orderby       = ' ORDER BY ' . $args['orderby'] . ' ' . $args['order'];
						$limit         = ' LIMIT ' . $args['offset'] . ', ' . (int) $args['limit'];
						// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Custom safe query as the variables in the query are already prepared.
			...
			if ( ! empty( $request_data['topic'] ) ) {
				...
				foreach ( $students as $student_id ) {
					...
					if ( $quiz_count > 0 ) {
						...
						// }
						$where         = 'statref.quiz_post_id IN (' . $quizzes_str . ')';
						$where        .= ' AND statref.course_post_id="' . esc_sql( $request_data['course'] ) . '"';
						$where        .= ' AND statref.user_id="' . esc_sql( $student_id ) . '"';
						$orderby       = ' ORDER BY ' . $args['orderby'] . ' ' . $args['order'];
						$limit         = ' LIMIT ' . $args['offset'] . ', ' . (int) $args['limit'];
                        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Custom safe query as the variables in the query are already prepared.
			...
			if ( isset( $request_data['group'] ) && ! empty( $request_data['group'] ) ) {
				// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- Optional group meta query.
				$query_args['meta_query'] = array(
					array(
						'key'     => 'learndash_group_enrolled_' . $request_data['group'],
						'compare' => 'EXISTS',
			...
			foreach ( $courses as $course ) {
				...
				if ( 0 < $quiz_count ) {
					...
					// }
					$where  = 'statref.quiz_post_id IN (' . $quizzes_str . ')';
					$where .= ' AND statref.course_post_id="' . esc_sql( $course->ID ) . '"';
			...
			foreach ( $accessible_courses as $course_id ) {
				...
				if ( $percentage >= $min_progress && $percentage < $max_progress ) {
					...
					$table_data[ get_the_title( $course_id ) ] = array(
						// phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date -- Intended use of date().
