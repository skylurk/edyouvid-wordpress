...
class QuizCompletionRate extends Component {
  ...
  render() {
    ...
    if(this.state.course_report_type == 'learner-specific-course-reports' && !wisdm_ld_reports_common_script_data.is_pro_version_active){
      body = <DummyReports image_path='qcr.png' url="https://www.learndash.com/reports-by-learndash"></DummyReports>;
      return (body);
    }
    let data_validation = '';
    ...
    <div class={"wisdm-learndash-reports-chart-block " + data_validation}> 
      <div class="wisdm-learndash-reports-quiz-completion-rate-per-course graph-card-container">
        ...
        <div class="chart-header quiz-completion-rate-per-course-chart-header">
          ...
          <ChartSummarySection wrapper_class='chart-summary-quiz-completion-rate-per-course' pro_upgrade_option={this.upgdare_to_pro} graph_summary={this.state.graph_summary} error={this.state.error} pro_link="https://www.learndash.com/reports-by-learndash"/>
        </div>
        <div>
          {graph}
        </div>
