			...
			if ( false === $data || ( defined( 'WRLD_DISABLE_TRANSIENTS' ) && WRLD_DISABLE_TRANSIENTS ) ) {
				...
				if ( $data ) {
					...
					if ( ! empty( $data['exclude_courses'] ) ) {
						// phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_post__not_in -- We set the post__not_in query arg only if it's passed in the data.
				...
				// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- We use GET data only to pass data and doesn't initiate any action.
				$ld_report_type = isset( $_GET['ld_report_type'] )
					// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- We use GET data only to pass data and doesn't initiate any action.
					? sanitize_text_field( wp_unslash( $_GET['ld_report_type'] ) )
					: null;

				// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- We use GET data only to pass data and doesn't initiate any action.
				$report = isset( $_GET['report'] )
					// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- We use GET data only to pass data and doesn't initiate any action.
					? sanitize_text_field( wp_unslash( $_GET['report'] ) )
					: null;

				if ( isset( $ld_report_type ) && 'quiz-reports' == $ld_report_type ) {
					if ( isset( $report ) && '' != $report ) {
						$qre_request_params['report'] = sanitize_text_field( wp_unslash( $report ) );
					}
				}
