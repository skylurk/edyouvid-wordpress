...
class Auto_Plugin_Install {
	...
	public function wp_ajax_auto_plugin_install() {
		...
		if ( empty( $data['action'] ) || empty( $data['slug'] ) || empty( $data['name'] ) ) {
			...
			$response->message = esc_html__( 'Please reload the page and try again.', 'uncanny-ceu' );
		...
		if ( 'install' === $action ) {
			...
			if ( $plugin_info->is_active ) {
				$response->success = false;
				$response->message = sprintf( esc_html__( '%s is already installed and active.', 'uncanny-ceu' ), $plugin_info->name );
	...
	public function install( $plugin_download_link = '', $activate_only = false, $plugin_basename = '' ) {
		...
		if ( false === $require_upgrader->success ) {
			...
			$process->message = sprintf( esc_html__( 'Could not install plugin. %s', 'uncanny-ceu' ), $require_upgrader->message );
		...
		if ( false === $require_skin->success ) {
			$process->success = false;
			$process->code    = 'required_files_not_found';
			$process->message = sprintf( esc_html__( 'Could not install plugin. %s', 'uncanny-ceu' ), $require_skin->message );
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			...
			$process->message = esc_html__( 'Could not install plugin. You do not have permission to install plugins.', 'uncanny-ceu' );
		...
		if ( ! wp_is_file_mod_allowed( __NAMESPACE__ . '_auto_plugin_install_can_install' ) ) {
			...
			$process->message = esc_html__( 'Could not install plugin. You do not have permission to modify files.', 'uncanny-ceu' );
		...
		if ( empty( $plugin_download_link ) ) {
			...
			$process->message = esc_html__( 'Could not install plugin. A plugin url was not received. Please Reload the page.', 'uncanny-ceu' );
		...
		if ( false === $this->has_credentials() ) {
			...
			$process->message = esc_html__( 'Could not install plugin. File system credentials could not be verified. Please Reload the page.', 'uncanny-ceu' );
		...
		if ( false === $activate_only ) {
			...
			if ( ! method_exists( $installer, 'install' ) ) {
				...
				$process->message = esc_html__( 'Could not install plugin. The installer failed to initialize.', 'uncanny-ceu' );
			...
			if ( empty( $plugin_basename ) ) {
				...
				$process->message = esc_html__( 'Could not install plugin. The download link failed. Please Reload the page.', 'uncanny-ceu' );
		...
		if ( is_wp_error( $activated ) ) {
			...
			$process->message = esc_html__( 'Plugin has installed but failed to activate. Please activate manually on the plugins page.', 'uncanny-ceu' );
		...
		if ( false !== $activate_only ) {
			...
			$process->message = esc_html__( 'Plugin has activated successfully.', 'uncanny-ceu' );
		...
		$process->message = esc_html__( 'Plugin has installed and activated successfully.', 'uncanny-ceu' );
	...
	public function button( $plugin_slug, $redirect = null ) {
		...
		if ( empty( $plugin_slug ) ) {
			return esc_html__( 'Add a plugin slug', 'uncanny-ceu' );
		}

		$plugin_info = $this->get_plugin_info( $plugin_slug );

		if ( false === $plugin_info ) {
			return esc_html__( 'Plugin not found on wordpress.org', 'uncanny-ceu' );
		}
