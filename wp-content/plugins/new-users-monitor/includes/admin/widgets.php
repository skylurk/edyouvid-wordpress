...
$debug_process = 'wp_dashboard_setup';

$User_Roles = array (
	'administrator',
);

$User_Roles = apply_filters( 'NewUsersMonitor_widget_user_roles', $User_Roles );
NUM_Debugger ($User_Roles, '$User_Roles', $debug_process, __FUNCTION__, __LINE__);

$is_CurrentUserCan_View = NUM_Check_User_Roles ($User_Roles);
NUM_Debugger ($is_CurrentUserCan_View, '$is_CurrentUserCan_View', $debug_process, __FUNCTION__, __LINE__);
		
if ( $is_CurrentUserCan_View ) {
	...
	function NUM_Dashboard_NewUsers() {
		...
		<script>
			...
			NUM_Widget_Header.innerHTML = '<span title="' + NUM_Widget_LinkTitle + '"><a href="/wp-admin/users.php?page=new-users-monitor/includes/admin/options.php" class="ulm_dashboard_widget_header">' + NUM_Widget_LinkCaption + '</a>' + NUM_Widget_Count_UnconfirmedUsers + '</span>';
		</script>		
	<?php }	