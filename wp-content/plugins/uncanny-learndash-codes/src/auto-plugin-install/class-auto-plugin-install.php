...
class Auto_Plugin_Install {
	...
	public function wp_ajax_auto_plugin_install() {
		...
		if ( empty( $data['action'] ) || empty( $data['slug'] ) || empty( $data['name'] ) ) {
			...
			$response->message = esc_html__( 'Please reload the page and try again.', 'uncanny-learndash-codes' );
		...
		if ( 'install' === $action ) {
			...
			if ( $plugin_info->is_active ) {
				$response->success = false;
				$response->message = sprintf( esc_html__( '%s is already installed and active.', 'uncanny-learndash-codes' ), $plugin_info->name );
	...
	public function install( $plugin_download_link = '', $activate_only = false, $plugin_basename = '' ) {
		...
		if ( false === $require_upgrader->success ) {
			...
			$process->message = sprintf( esc_html__( 'Could not install plugin. %s', 'uncanny-learndash-codes' ), $require_upgrader->message );
		...
		if ( false === $require_skin->success ) {
			$process->success = false;
			$process->code    = 'required_files_not_found';
			$process->message = sprintf( esc_html__( 'Could not install plugin. %s', 'uncanny-learndash-codes' ), $require_skin->message );
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			...
			$process->message = esc_html__( 'Could not install plugin. You do not have permission to install plugins.', 'uncanny-learndash-codes' );
		...
		if ( ! wp_is_file_mod_allowed( __NAMESPACE__ . '_auto_plugin_install_can_install' ) ) {
			...
			$process->message = esc_html__( 'Could not install plugin. You do not have permission to modify files.', 'uncanny-learndash-codes' );
		...
		if ( empty( $plugin_download_link ) ) {
			...
			$process->message = esc_html__( 'Could not install plugin. A plugin url was not received. Please Reload the page.', 'uncanny-learndash-codes' );
		...
		if ( false === $this->has_credentials() ) {
			...
			$process->message = esc_html__( 'Could not install plugin. File system credentials could not be verified. Please Reload the page.', 'uncanny-learndash-codes' );
		...
		if ( false === $activate_only ) {
			...
			if ( ! method_exists( $installer, 'install' ) ) {
				...
				$process->message = esc_html__( 'Could not install plugin. The installer failed to initialize.', 'uncanny-learndash-codes' );
			...
			if ( empty( $plugin_basename ) ) {
				...
				$process->message = esc_html__( 'Could not install plugin. The download link failed. Please Reload the page.', 'uncanny-learndash-codes' );
		...
		if ( is_wp_error( $activated ) ) {
			...
			$process->message = esc_html__( 'Plugin has installed but failed to activate. Please activate manually on the plugins page.', 'uncanny-learndash-codes' );
		...
		if ( false !== $activate_only ) {
			...
			$process->message = esc_html__( 'Plugin has activated successfully.', 'uncanny-learndash-codes' );
		...
		$process->message = esc_html__( 'Plugin has installed and activated successfully.', 'uncanny-learndash-codes' );
	...
	public function button( $plugin_slug, $redirect = null ) {
		...
		if ( empty( $plugin_slug ) ) {
			return esc_html__( 'Add a plugin slug', 'uncanny-learndash-codes' );
		}

		$plugin_info = $this->get_plugin_info( $plugin_slug );

		if ( false === $plugin_info ) {
			return esc_html__( 'Plugin not found on wordpress.org', 'uncanny-learndash-codes' );
		}
		...
		if ( 'uncanny-automator' === (string) $plugin_slug ) {
			$button_text = esc_html__( 'Install Uncanny Automator', 'uncanny-learndash-codes' );
		} else {
			$button_text = sprintf( esc_html__( 'Install %s', 'uncanny-learndash-codes' ), $plugin_info->name );
		}
		...
		if ( $plugin_info->is_active ) {
			...
			$button_text = sprintf( esc_html__( '%s is active', 'uncanny-learndash-codes' ), $plugin_info->name );
			$button_text = apply_filters( 'uncanny_one_click_install_plugin_active_text', $button_text, $plugin_info );
		} elseif ( $plugin_info->is_installed ) {
			$action = esc_attr( 'activate' );
			if ( 'uncanny-automator' === (string) $plugin_slug ) {
				$button_text = esc_html__( 'Activate Uncanny Automator', 'uncanny-learndash-codes' );
			} else {
				$button_text = sprintf( esc_html__( 'Activate %s', 'uncanny-learndash-codes' ), $plugin_info->name );
			}
