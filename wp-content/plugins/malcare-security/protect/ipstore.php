...
class MCProtectIpstore_V662 {
	...
	function __construct($storage_type = MCProtectIpstore_V662::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == MCProtectIpstore_V662::STORAGE_TYPE_FS) {
			$this->storage = new MCProtectIpstoreFS_V662();
		} else {
			$this->storage = new MCProtectIpstoreDB_V662();
		}
	}

	public static function uninstall() {
		MCProtectIpstoreDB_V662::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == MCProtectIpstore_V662::STORAGE_TYPE_DB) {
