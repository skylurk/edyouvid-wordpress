...
class LearndashGroupsPostEditAdditions {
	...
	public function __construct() {
		...
		// Code already exists in uncanny_learndash_groups\ProcessGroupDeletion
		// downgrade group
	...
	public function render_metabox( $post ) {
		...
		if ( ! empty( $code_group_id ) && empty( get_post_meta( $post->ID, SharedFunctions::$code_group_downgraded, true ) ) ) {
			...
			if ( false !== $render_reconcile_seats ) {
				...
				if ( empty( get_post_meta( $post->ID, SharedFunctions::$code_group_downgraded, true ) ) ) {
					$link_downgrade = admin_url( 'admin.php?downgrade-group=true&group-id=' . $post->ID . '&_wpnonce=' . wp_create_nonce( 'Uncanny0wL' ) );
					?>
					<hr/>
					<div class="sfwd_input"
						id="sfwd-lessons_upgrade_learndash_group">
				<span class="sfwd_option_input" style="width:100%;">
					<div class="sfwd_option_div">
						<a id="downgrade_group"
							onclick="javascript: return confirm('<?php echo __( 'This action cannot be reversed.', 'uncanny-learndash-groups' ); ?>')"
							href="<?php echo $link_downgrade; ?>"
							class="button button-secondary"><?php _e( 'Downgrade this group', 'uncanny-learndash-groups' ); ?></a>
					</div>
					<div class="sfwd_help_text_div"
						id="uo-total-seats-count">
						<label
							class="sfwd_help_text">
							<strong><?php printf( __( 'Warning: Downgrading this group to "basic" will mean %s, courses and users can no longer be managed by Group Leaders and enrollment keys will be deleted.', 'uncanny-learndash-groups' ), strtolower( ulgm()->group_management->seat->get_per_seat_text( 5 ) ) ); ?></strong>
						</label>
					</div>
				</span>
						<p style="clear:left"></p>
			...
			<div class="sfwd_input"
				...
				?>
				<p style="clear:left"><?php printf( '%s %s.', __( 'Clicking this button will enable seat management for this group and enable front end management using Uncanny Groups for LearnDash. To bulk upgrade all groups, use the button near the bottom of Uncanny Groups >', 'uncanny-learndash-groups' ), $url ); ?></p>
			</div>
			<?php
		}
		?>
	...
	public function render_metabox_for_woo( $post ) {
		...
		if ( ! empty( get_post_meta( $post->ID, SharedFunctions::$code_group_downgraded, true ) ) ) {
			...
			if ( ! empty( $time ) ) {
				?>
				<h4><?php printf( '%s: %s', esc_html__( 'This group was downgraded on', 'uncanny-learndash-groups' ), date_i18n( get_option( 'date_format', $time ) ) ); ?></h4>
				<?php
			}
		...
		<ul>
			<li>
				<?php printf( '<h4>%s: <a href="%s">#%d %s %s</a> <i>(Status: %s)</i></h4>', esc_html__( 'Main order', 'uncanny-learndash-groups' ), get_edit_post_link( $order_id ), $order_id, $order->get_billing_first_name(), $order->get_billing_last_name(), $order->get_status() ); ?>
			</li>
		</ul>
		<h4><?php printf( '%s: <a href="%s">%s</a>', esc_html__( 'Group license', 'uncanny-learndash-groups' ), get_edit_post_link( $product_id ), get_the_title( $product_id ) ); ?></h4>
		<?php
		...
		<ol>
			...
			foreach ( $order_ids as $o_id ) {
				...
				?>
				<li>
					<?php printf( '<a href="%s">#%d %s %s</a><i>%s</i>', get_edit_post_link( $o_id ), $o_id, $ordr->get_billing_first_name(), $ordr->get_billing_last_name(), ' &mdash; ' . $order_type . '' ); ?>
				</li>
			<?php } ?>
		</ol>
		<?php
	 ...
	 */
	public static function update_user_redeemed_seat_func( $post_id, $force = false, $group_users = array(), $group_admins = array() ) {
		// For child groups with pooled seats, we need to use the parent group's code_group_id
		$code_group_id = ulgm()->group_management->seat->get_code_group_id( $post_id );
		if ( empty( $code_group_id ) ) {
			return;
		}

		// Handle child groups with pooled seats by using parent's code_group_id
		if ( ! SharedFunctions::is_a_parent_group( $post_id ) && SharedFunctions::is_pool_seats_enabled_for_current_parent_group( $post_id ) ) {
			$parent_group_id      = ulgm()->group_management->seat->get_real_ld_group_id( $post_id );
			$parent_code_group_id = ulgm()->group_management->seat->get_code_group_id( $parent_group_id );
			if ( ! empty( $parent_code_group_id ) ) {
				$code_group_id = $parent_code_group_id;
			}
		}

		// Smart processing - only rebuild all assignments when force=true
		// When force=false, we only process specific users that changed
		if ( ! $force ) {
			// Only process the specific users that were passed in
			// This prevents clearing existing seat assignments
			...
			// Only add seats if we need more
			$number_of_seats_needed = count( $seat_users ) + count( $seat_groups_leaders );
			$existing_seats         = ulgm()->group_management->seat->total_seats( $post_id );

			if ( $number_of_seats_needed > $existing_seats ) {
				$diff      = $number_of_seats_needed - $existing_seats;
			...
			// Only assign seats to new users, preserve existing assignments
			// Process group leaders first
			if ( ! empty( $seat_groups_leaders ) ) {
				foreach ( $seat_groups_leaders as $user ) {
					// Check if this user already has a seat code assigned
					// For pooled seats, check against the parent group's pooled seat ID, not the child group ID
					$check_group_id = SharedFunctions::is_pool_seats_enabled_for_current_parent_group( $post_id ) ?
						ulgm()->group_management->seat->get_real_ld_group_id( $post_id ) : $post_id;
					$existing_code  = ulgm()->group_management->seat->if_user_redeemed_code( $user['ID'], $check_group_id );
					if ( empty( $existing_code ) ) {
						self::directly_assign_seat_to_user( $user['ID'], $post_id, $code_group_id, SharedFunctions::$redeem_status );
					}
				}
			}
			...
			if ( ! empty( $seat_users ) ) {
				foreach ( $seat_users as $user ) {
					...
					$existing_code  = ulgm()->group_management->seat->if_user_redeemed_code( $user['ID'], $check_group_id );
					if ( empty( $existing_code ) ) {
						self::directly_assign_seat_to_user( $user['ID'], $post_id, $code_group_id, SharedFunctions::$not_started_status );
					}
				}
			}
		} else {
			// Force=true means rebuild all seat assignments (legacy behavior)
			...
			$number_of_seats = count( $seat_users ) + count( $seat_groups_leaders );
			$existing_seats  = ulgm()->group_management->seat->total_seats( $post_id );
			...
			// Process all users (this will rebuild all assignments)
			if ( count( $seat_users ) || count( $seat_groups_leaders ) ) {
	...
	public static function remove_users_from_group_admin_func( $post_id, $force = false, $group_leaders = array(), $group_users = array() ) {
		...
		// When force=false, we're only processing specific changes and shouldn't clear existing assignments
		// This prevents unnecessary clearing of seat codes on every save
		if ( ! $force ) {
			return;
		}

		// Only proceed with clearing assignments when force=true (legacy behavior)
		...
		if ( count( $codes_users ) ) {
			foreach ( $codes_users as $user_id => $code_id ) {
				if ( ! key_exists( $user_id, $seat_users ) ) {
					$sql = $wpdb->prepare( "UPDATE {$wpdb->prefix}" . SharedFunctions::$db_group_codes_tbl . ' SET student_id = NULL, user_email = NULL, used_date = NULL, code_status = %s, ld_group_id = NULL WHERE ID = %d', SharedFunctions::$available_status, $code_id );
					$wpdb->query( $sql );
				}
			}
		}
...
AND group_id =%d',
		...
		if ( $code_group_users ) {
			...
			foreach ( $code_group_users as $u ) {
				$ld_group_id = $u->ld_group_id;
				if ( ! isset( $codes_users[ $ld_group_id ][ $u->user_email ] ) ) {
					$codes_users[ $ld_group_id ][ $u->user_email ] = $u->ID;
				} else {
					// this email is used twice then let's recover old code
					$code_id = $codes_users[ $ld_group_id ][ $u->user_email ];
					$sql     = $wpdb->prepare(
						"UPDATE {$wpdb->prefix}" . SharedFunctions::$db_group_codes_tbl . '
					SET student_id = NULL,
					user_email = NULL,
					used_date = NULL,
					code_status = %s,
...
AND group_id = %d',
		...
		if ( $code_group_users ) {
			...
			foreach ( $code_group_users as $u ) {
				...
				if ( isset( $codes_users[ $ld_group_id ][ $student->user_email ] ) ) {
					$code_id = $codes_users[ $ld_group_id ][ $student->user_email ];
					$sql     = $wpdb->prepare(
						"UPDATE {$wpdb->prefix}" . SharedFunctions::$db_group_codes_tbl . '
						SET student_id = NULL,
						user_email = NULL,
				...
				if ( ! isset( $codes_users_ids[ $ld_group_id ][ $u->student_id ] ) ) {
					$codes_users_ids[ $ld_group_id ][ $u->student_id ] = $u->ID;
				} else {
					// this email is used twice then let's recover old code
					$code_id = $codes_users_ids[ $ld_group_id ][ $u->student_id ];
					$sql     = $wpdb->prepare(
						"UPDATE {$wpdb->prefix}" . SharedFunctions::$db_group_codes_tbl . '
						SET student_id = NULL,
						user_email = NULL,
	...
	public function perform_group_management_actions( $post_id, $post ) {
		...
		// We now handle child groups with pooled seats properly in update_user_redeemed_seat_func
		// so we don't need to block them here
		 ...
		 * Only run this when there are actual changes to users
		 */
		$r = $this->update_user_redeem_seats( $post->ID );
		if ( empty( $r ) ) {
			// No changes to users, don't run seat management functions
			return;
		}
		...
		/**
		 * Remove users -- Start
		 * Only run this when there are actual changes, and NOT with force=true
		 * to prevent clearing existing seat assignments
		 */
		if ( ! empty( $group_users ) || ! empty( $group_admins ) ) {
			// Only remove users that are no longer in the group, don't clear all assignments
			self::remove_users_from_group_admin_func( $post->ID, false, $group_admins, $group_users );
		}
		/**
		 * Remove users -- End
		 */
	...
	public static function update_seat_count( $post_id, $code_group_id, $new_seats = null ) {
		// Skip seat count updates for child groups with pooled seats
		// Their seat counts are managed by the parent group
		if ( ! SharedFunctions::is_a_parent_group( $post_id ) && SharedFunctions::is_pool_seats_enabled_for_current_parent_group( $post_id ) ) {
			return;
		}
	...
	public function update_user_redeem_seats( $post_id ) {
		...
		// Only call update_user_redeemed_seat_func if there are actual changes
		if ( ! empty( $group_users ) || ! empty( $group_admins ) ) {
			// Pass force=false to prevent the function from fetching all users and reassigning seats
			self::update_user_redeemed_seat_func( $post_id, false, $group_users, $group_admins );
			if ( ! SharedFunctions::is_a_parent_group( $post_id ) && SharedFunctions::is_pool_seats_enabled_for_current_parent_group( $post_id ) ) {
	...
	public function get_changed_users_admins( $post_id ) {
		...
		// Check for the specific change indicators from LearnDash binary selector
		$users_changed_key   = 'learndash_group_users-' . $post_id . '-changed';
		$leaders_changed_key = 'learndash_group_leaders-' . $post_id . '-changed';

		// Check for group users changes
		if ( ( isset( $_POST[ $users_changed_key ] ) ) && ( ! empty( $_POST[ $users_changed_key ] ) ) ) {
			// Get the current group users from POST data
			if ( isset( $_POST['learndash_group_users'] ) && isset( $_POST['learndash_group_users'][ $post_id ] ) ) {
				$group_user_ids = $_POST['learndash_group_users'][ $post_id ];

				if ( ! empty( $group_user_ids ) ) {
					// Handle both array and JSON string formats from LearnDash selector
					if ( is_string( $group_user_ids ) ) {
						$group_user_ids = json_decode( stripslashes( $group_user_ids ), true );
					}
		...
		$group_admins = array();
		// Check for group leaders changes
		if ( ( isset( $_POST[ $leaders_changed_key ] ) ) && ( ! empty( $_POST[ $leaders_changed_key ] ) ) ) {
			// Get the current group leaders from POST data
			if ( isset( $_POST['learndash_group_leaders'] ) && isset( $_POST['learndash_group_leaders'][ $post_id ] ) ) {
				$group_admin_ids = $_POST['learndash_group_leaders'][ $post_id ];

				if ( ! empty( $group_admin_ids ) ) {
					// Handle both array and JSON string formats from LearnDash selector
					if ( is_string( $group_admin_ids ) ) {
						$group_admin_ids = json_decode( stripslashes( $group_admin_ids ), true );
					}

					if ( is_array( $group_admin_ids ) && ! empty( $group_admin_ids ) ) {
	...
	public function downgrade_selected_group() {
		...
		// New functionality
		$r = ProcessGroupDeletion::remove_related_groups_data( $group_id );

		// Only if the codes & groups deleted, then add the downgrade key
		if ( $r ) {
			$wpdb->query( $wpdb->prepare( "UPDATE $wpdb->prefix" . ulgm()->db->tbl_group_details . ' SET order_id = %d WHERE ID = %d AND ld_group_id = %d', $order_id, $code_group_id, $group_id ) ); //phpcs: ignore
			update_post_meta( $group_id, SharedFunctions::$code_group_downgraded, 'yes' );
			update_post_meta( $group_id, SharedFunctions::$code_group_downgraded . '_time', current_time( 'timestamp' ) ); //phpcs: ignore
		}
	...
	/**
	 * Directly assign a seat code to a user in the database
	 * This bypasses the problematic Group_Management_Helpers::add_existing_user function
	 */
	private static function directly_assign_seat_to_user( $user_id, $group_id, $code_group_id, $status = 'redeem' ) {
		global $wpdb;

		// Find an available seat code
		$available_code = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT ID, code FROM {$wpdb->prefix}" . ulgm()->db->tbl_group_codes . ' 
			WHERE group_id = %d AND code_status = %s AND student_id IS NULL LIMIT 1',
				$code_group_id,
				'available'
			)
		);
		...
		// Assign the seat code to the user
		// Note: ld_group_id should be the actual group where the user was added (could be child group)
		// but the seat code belongs to the parent group (code_group_id)
		$result = $wpdb->update(
			...
			array(
				...
				'code_status' => $status,
				'ld_group_id' => $group_id, // This is the actual group where user was added
			),
			array( 'ID' => $available_code->ID ),
			array( '%d', '%s', '%s', '%s', '%d' ),
			array( '%d' )
		);
