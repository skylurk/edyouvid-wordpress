...
class Group_Management_DB_Handler {
	 ...
	 * @return int
	 */
	public function count_users_enrolled_in_group_for_seats( $ld_group_id ) {
		$users = learndash_get_groups_users( $ld_group_id, true );
		if ( ( SharedFunctions::is_a_parent_group( $ld_group_id ) || SharedFunctions::has_children_in_group( $ld_group_id ) ) && SharedFunctions::is_pool_seats_enabled_for_current_parent_group( $ld_group_id, false ) ) {
			$group_children = learndash_get_group_children( $ld_group_id );
			$children_users = array();
	 ...
	 */
	public function get_ld_group_id_from_code_group( $code ) {
		if ( empty( $code ) ) {
			return 0;
		}

		global $wpdb;
		$tbl_group = ulgm()->db->tbl_group_details;
		$tbl_codes = ulgm()->db->tbl_group_codes;

		return $wpdb->get_var(
			"SELECT g.ld_group_id
FROM $wpdb->prefix{$tbl_group} g
JOIN $wpdb->prefix{$tbl_codes} c
ON c.group_id = g.ID AND c.code = %s",
			$code
		);
	}
	...
	public function set_sign_up_code_status( $data = array() ) {
		...
		$ld_group_id = (int) $data['ld_group_id'] ?? $this->get_ld_group_id_from_code_group( $code );
		...
		if ( is_numeric( $ld_group_id ) && 0 !== absint( $ld_group_id ) ) {
			$update = $wpdb->update(
				$wpdb->prefix . ulgm()->db->tbl_group_codes,
				array( 'ld_group_id' => $ld_group_id ),
				array( 'code' => $code ),
				array( '%d' ),
				array( '%s' )
			);
	...
	public function is_key_available( $key ) {
		...
		$results = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT gc.*, gd.ld_group_id as gd_ld_group_id
		...
		$code_id        = $results->ID; // Group Codes ID
		$student_id     = $results->student_id; // Group Codes Student/ ID
		$gd_ld_group_id = $results->gd_ld_group_id; // Group Details LD Group ID
		$ld_group_id    = $results->ld_group_id; // Group Codes LD Group ID
		$code_group_id  = $results->group_id; // Group Codes Group/Batch ID
		$code_status    = $results->code_status; // Group Codes Code Status

		$learndash_group_id = (int) $ld_group_id;
		if ( empty( $ld_group_id ) || 0 === $ld_group_id ) {
			$learndash_group_id = (int) $gd_ld_group_id;
		}

		if ( 0 === (int) $this->seat->remaining_seats( $learndash_group_id ) && SharedFunctions::$not_redeemed_status !== (string) $code_status ) {
			return array(
				'result' => 'failed',
		...
		return array(
			'result'         => 'success',
			'code_id'        => $code_id,
			'ld_group_id'    => empty( $ld_group_id ) ? $learndash_group_id : $ld_group_id,
			'gd_ld_group_id' => empty( $gd_ld_group_id ) ? $learndash_group_id : $gd_ld_group_id,
...
WHERE c.student_id = %d $limit",
	...
	public function get_temp_email_from_code( $code, $group_id, $user_id = 0 ) {
		...
		return $wpdb->get_var( $wpdb->prepare( 'SELECT user_email FROM ' . $wpdb->prefix . ulgm()->db->tbl_group_codes . ' WHERE code LIKE %s AND group_id = %d AND code_status = %s', $code, $codes_group_id, SharedFunctions::$not_redeemed_status ) );
	}
...
WHERE student_id IS NULL AND user_email IS NULL",
	...
	public function check_sign_up_code_from_group_id( $group_id = null, $user_email = '' ) {
		...
		return $wpdb->get_var( $wpdb->prepare( "SELECT `code` FROM $wpdb->prefix" . ulgm()->db->tbl_group_codes . ' WHERE student_id IS NULL AND user_email LIKE %s AND code_status = %s AND group_id=%d LIMIT %d', $user_email, SharedFunctions::$not_redeemed_status, $codes_group_id, 1 ) );
	}
	...
	public function users_invited_in_group( $ld_group_id, $count = true ) {
		...
		global $wpdb;
		if ( $count ) {
			$qry = $wpdb->prepare( 'SELECT COUNT(ID) AS remaining FROM ' . $wpdb->prefix . ulgm()->db->tbl_group_codes . ' WHERE student_id IS NULL AND user_email IS NOT NULL AND code_status = %s AND group_id = %d AND ld_group_id = %d', SharedFunctions::$not_redeemed_status, $group_id, $ld_group_id );
		...
		$qry = $wpdb->prepare( 'SELECT `code`, `user_email`, `first_name`, `last_name` FROM ' . $wpdb->prefix . ulgm()->db->tbl_group_codes . ' WHERE student_id IS NULL AND user_email IS NOT NULL AND code_status = %s AND group_id = %d AND ld_group_id = %d', SharedFunctions::$not_redeemed_status, $group_id, $ld_group_id );
