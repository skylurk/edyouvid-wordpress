...
class Woocommerce_Handlers {
	...
	public function get_group_leader_all_orders( $group_id, $product_id = null ) {
		...
		if ( is_numeric( $product_id ) ) {
			$qry = $wpdb->prepare(
				"SELECT wpp.order_item_id as OrderPrimaryID,wpp.order_id as OrderID,
										...
										AND wpu.ID = %d AND wpm.meta_value = %d",
		...
		$qry = $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key = %s AND meta_value = %d", SharedFunctions::$linked_group_id_meta, $group_id );
	...
	public function get_product_id_from_group_id( $group_id = null ) {
		...
		$product_id = get_post_meta( $group_id, '_ulgm_license_product_id', true );

		if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			$order = wc_get_order( $order_id );
			if ( ( empty( $product_id ) || 0 === $product_id ) && $order ) {
