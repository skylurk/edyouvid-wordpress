...
class SharedFunctions {
	...
	public function __construct() {
		add_action( 'init', array( $this, 'set_translation_properties' ), PHP_INT_MAX );
	}

	public function set_translation_properties() {
		self::$not_started_status = __( 'Not Started', 'learndash' );
	}
	 ...
	 * @return null
	 */
	public static function set_sign_up_code_status( $code, $user_id = null, $order_id = null, $code_status = null, $group_id = null, $user_email = null, $first_name = '', $last_name = '', $ld_group_id = 0 ) {
		$data = array(
			'code'        => $code,
			'user_id'     => $user_id,
			'order_id'    => $order_id,
	...
	public static function set_transient_cache( $cache, $data = array(), $user_id = null ) {
		...
		if ( is_multisite() ) {
			...
			add_option( $cache, $data, '', 'no' ); // 'no' prevents autoloading
		}
	}
	 ...
	 * @depreacated v6.0.1
	 */
	public static function set_user_to_code( $user_id, $code, $code_status = '', $ld_group_id = 0 ) {
		if ( empty( $code_status ) ) {
			$code_status = self::$redeem_status;
		}

		return ulgm()->group_management->set_user_to_code( $user_id, $code, $code_status, $ld_group_id );
	}
	...
	public static function get_user_current_progress_in_group( $user_id, $group_id = null ) {
		...
		if ( $completed ) {
			...
			$status = __( 'In Progress', 'learndash' );
		} else {
	 ...
	 * @return mixed|string|void
	 */
	public static function ulgm_group_management_email_users_from_email( $group_id = null, $user_id = 0 ) {
	...
	public static function load_backend_bundles() {
		...
		// Check Post Types.
		if ( 'post' === $screen->base ) {
			if ( in_array( $screen->id, array( 'groups', 'product' ), true ) ) {
				return true;
			}
		}

		// Check Pages.
		return preg_match( '/uncanny(\-learndash)?\-groups(.+)?/', $screen->base, $matches );
	}
	 ...
	 * @return array
	 */
	public static function get_courses_from_license_product( $product_id ) {
		$product = wc_get_product( $product_id );
		if ( ! ( $product instanceof \WC_Product ) ) {
			return array();
		}
		if ( 'courses' === $product->get_type() ) {
			return array( get_post_meta( $product_id, '_ulgm_course', true ) );
		}
		if ( 'license' !== $product->get_type() ) {
			return array();
		}
		$courses_linked = get_post_meta( $product_id, self::$license_meta_field, true );
		$course_ids     = array();
		if ( $courses_linked ) {
			foreach ( $courses_linked as $course_product ) {
				$course_ids[] = get_post_meta( $course_product, '_ulgm_course', true );
			}
		}
		return $course_ids;
	}
	 ...
	 * @return string
	 */
	public static function generate_fallback_group_title( \WC_Order $order ) {
		return sprintf( '%s %s %s', $order->get_billing_first_name(), $order->get_billing_last_name(), $order->get_billing_company() );
	}
}
