...
class GroupManagementInterface {
	...
	public function register_scripts() {
		...
		// File saving utility
		wp_register_script( 'ulgm-file-saver', Utilities::get_vendor( 'filesaver/js/filesaver.js' ), array(), Utilities::get_version(), true );

		// Modal
		wp_register_script( 'ulgm-modal', Utilities::get_vendor( 'jquery-modal/js/jquery.modal.js' ), array( 'jquery' ), Utilities::get_version(), true );
		wp_register_script( 'ulgm-select2', Utilities::get_vendor( 'select2/js/select2.min.js' ), array( 'jquery' ), Utilities::get_version(), true );
		wp_register_style( 'ulgm-modal', Utilities::get_vendor( 'jquery-modal/css/jquery.modal.css' ), array(), Utilities::get_version() );
		wp_register_style( 'ulgm-select2', Utilities::get_vendor( 'select2/css/select2.min.css' ), array(), Utilities::get_version() );
	}
	...
	public function uo_group_mgr( $attributes, $content = '' ) {
		...
		*/
		...
		$edit_page_id        = $attributes['edit_page_id'];
	...
	public static function set_group_id() {
		...
		self::$ulgm_managed_group_objects = $group_leader_groups;
		...
		$user_group_ids = array_column( $group_leader_groups, 'ID' );
		...
		$first_group = 0;
		// always pre-select parent group.
		if ( isset( self::$ulgm_managed_group_objects[0] ) ) {
			if ( self::$ulgm_managed_group_objects[0]->post_parent > 0 ) {
				$parent_group = self::$ulgm_managed_group_objects[0]->post_parent;
				if ( SharedFunctions::can_user_manage_this_group( $group_leader_id, $parent_group, $user_group_ids ) ) {
					$first_group = $parent_group;
				} else {
					$first_group = self::$ulgm_managed_group_objects[0]->ID;
				}
			} else {
				$first_group = self::$ulgm_managed_group_objects[0]->ID;
			}
	...
	public static function set_enrolled_users_data( $group_id = null, $args = array() ) {
			...
			foreach ( $groups_user_object as $user ) {
				...
				if ( $learndash_group_enrolled_courses ) {
					foreach ( $learndash_group_enrolled_courses as $course_id ) {
						...
						} elseif ( isset( $progress[ $course_id ] ) && isset( $progress[ $course_id ]['status'] ) && 'completed' === (string) strtolower( $progress[ $course_id ]['status'] ) ) {
								$completed = true;
				...
				if ( $in_progress && ! $not_started && ! $completed ) {
					...
					$status    = '<a class="status status-in-progress" data-status="in-progress" href="' . $user_progress_link . '?user-id=' . $user->ID . '&group-id=' . self::$ulgm_current_managed_group_id . '">' . __( 'In Progress', 'learndash' ) . '</a>';
				}
				...
				if ( $completed && ! $not_started && ! $in_progress ) {
					$status_id = 'completed';
					$status    = '<a class="status status-completed" data-status="completed" href="' . $user_progress_link . '?user-id=' . $user->ID . '&group-id=' . self::$ulgm_current_managed_group_id . '">' . __( 'Completed', 'learndash' ) . '</a>';
				}
				...
				if ( false === $status ) {
					if ( 'yes' === $use_progress_report_instead_course ) {
						...
						$status    = '<a class="status status-not-started" data-status="not-started" href="' . $user_progress_link . '?user-id=' . $user->ID . '&group-id=' . self::$ulgm_current_managed_group_id . '">' . __( 'Not Started', 'learndash' ) . '</a>';
		...
		$temp_users_code = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->prefix" . SharedFunctions::$db_group_codes_tbl . ' WHERE ld_group_id = %d AND code_status = %s AND student_id IS NULL', $group_id, SharedFunctions::$not_redeemed_status ) );
	...
	/**
	 * This function is used to reconcile the parent and child group seats when pooling is enabled
	 *
	 * @param int $group_id The ID of the group to reconcile
	 * @param int $total_seats The total number of seats in the group
	 * @param int $enrolled_seats The number of enrolled users in the group
	 * @param int $remaining_seats The number of remaining seats in the group
	 * @return bool True if the group seats were reconciled, false otherwise
	 */
	public static function reconcile_pooled_group_seats( $group_id, $total_seats, $enrolled_seats, $remaining_seats ) {

		// Lets not do this for now as its creating an issue, where child group users are added to the parent group

		// $ld_group_id = ulgm()->group_management->seat->get_real_ld_group_id( $group_id );
		// $all_group_ids = learndash_get_group_children( $ld_group_id );
		// $all_group_ids[] = $ld_group_id;
		...
		//  $group_leaders = LearndashFunctionOverrides::learndash_get_groups_administrators( $group_id, true );
		//  $all_group_leaders = array_merge( $all_group_leaders, $group_leaders );
		//  $group_users = LearndashFunctionOverrides::learndash_get_groups_users( $group_id, true );
		//  $all_group_users = array_merge( $all_group_users, $group_users );
		// }

		// LearndashGroupsPostEditAdditions::update_user_redeemed_seat_func( $group_id, $force, $all_group_users, $all_group_leaders );
	...
	public static function localize_filter_globalize_text() {
		...
		if ( self::is_reconcile_required( $group_post->ID, $total_seats, $enrolled_seats, $remaining_seats, true ) ) {
			$total_seats    = (int) ulgm()->group_management->seat->total_seats( $group_post->ID );
			$enrolled_seats = (int) ulgm()->group_management->count_users_enrolled_in_group( $group_post->ID ) + (int) ulgm()->group_management->users_invited_in_group( $group_post->ID );
			// Recalculate remaining seats after reconciliation
			$remaining_seats = (int) ulgm()->group_management->seat->remaining_seats( $group_post->ID );
		}
		...
		self::$ulgm_management_shortcode['text']['error'] = esc_html__( 'Error', 'uncanny-learndash-groups' );
		...
		self::$ulgm_management_shortcode['text']['modal_x_seats_remaining']           = sprintf( _x( 'You have %1$s %2$s remaining', '%1$s is a number, %2$s is the "seats" label', 'uncanny-learndash-groups' ), $remaining_seats, strtolower( get_option( 'ulgm_per_seat_text_plural', 'seats' ) ) );
