...
class Group_Management_Helpers {
	...
	public static function add_invite_user( $user_data, $do_not_send_emails = false, $counter = false, $is_api = true ) {
		...
		if ( $username_column_exists && ! empty( $user_data['username'] ) && mb_strlen( $user_data['username'] ) > 60 ) {
			if ( ! $counter ) {
				$data['message'] = esc_html__( 'Username is too long.', 'uncanny-learndash-groups' );
				wp_send_json_error( $data );
			} else {
				return array(
					'error-code' => __( 'Username is too long.', 'uncanny-learndash-groups' ),
					'status'     => array(
						'user'   => 'new_failed',
						// new_created_added_to_group, new_failed, existing_added_to_group, existing_already_member, existing_failed
						'email'  => 'not_sent',
		...
		$sanitized_user_name = '';
		if ( $username_column_exists && ! empty( $user_data['username'] ) ) {
			$sanitized_user_name = sanitize_user( $user_data['username'] );
			if ( strtolower( $sanitized_user_name ) !== strtolower( $user_data['username'] ) ) {
				if ( ! $counter ) {
					$data['message'] = esc_html__( 'Username has illegal characters.', 'uncanny-learndash-groups' );
					wp_send_json_error( $data );
				} else {
					return array(
						'error-code' => __( 'Username has illegal characters.', 'uncanny-learndash-groups' ),
						'status'     => array(
							'user'   => 'new_failed',
							// new_created_added_to_group, new_failed, existing_added_to_group, existing_already_member, existing_failed
							'email'  => 'not_sent',
		...
		$username_exists = ! empty( $sanitized_user_name ) ? username_exists( $user_data['user_login'] ) : username_exists( $user_data['user_email'] );

		if ( $username_column_exists && ! empty( $sanitized_user_name ) && false === $email_exists ) {
			if ( username_exists( $sanitized_user_name ) ) {
				if ( ! $counter ) {
					$data['message'] = esc_html__( 'This username already exists.', 'uncanny-learndash-groups' );
					wp_send_json_error( $data );
				} else {
					return array(
						'error-code' => __( 'This username already exists.', 'uncanny-learndash-groups' ),
						'status'     => array(
							'user'   => 'new_failed',
							// new_created_added_to_group, new_failed, existing_added_to_group, existing_already_member, existing_failed
							'email'  => 'not_sent',
		...
		if ( $email_exists || $username_exists ) {
			...
			if ( $username_column_exists && isset( $user_data['username'] ) && ! empty( $user_data['username'] ) && strtolower( $user_object->user_login ) !== strtolower( $user_data['username'] ) ) {

				if ( ! $counter ) {
					$data['message'] = esc_html__( 'A user was found with a matching email address; however, the username in WordPress does not match the username in the spreadsheet. No update was made to this user.', 'uncanny-learndash-groups' );
				} else {
					return array(
						'error-code' => __( 'A user was found with a matching email address; however, the username in WordPress does not match the username in the spreadsheet. No update was made to this user.', 'uncanny-learndash-groups' ),
						'status'     => array(
			...
			if ( ! current_user_can( 'manage_options' ) && $user_object && user_can( $user_object, 'manage_options' ) ) {
				if ( ! $counter ) {
					$data['message'] = esc_html__( 'You are not authorized to add this user to your group.', 'uncanny-learndash-groups' );
					wp_send_json_error( $data );
				} else {
					return array(
						'error-code' => __( 'You are not authorized to add this user to your group.', 'uncanny-learndash-groups' ),
						'status'     => array(
							'user'   => 'existing_failed',
							// new_created_added_to_group, new_failed, existing_added_to_group, existing_already_member, existing_failed
							'email'  => 'not_sent',
		...
		$password_info = '';
		if ( key_exists( 'user_pass', $user_data ) && ! empty( $user_data['user_pass'] ) && strlen( $user_data['user_pass'] ) >= $password_length ) {
			$plain_password = $user_data['user_pass'];

			$password_info = sprintf( __( 'Password did not include at least %1$d characters and was replaced with an auto-generated password.', 'uncanny-learndash-groups' ), $password_length );
		...
		if ( is_wp_error( $user_id ) ) {
			...
			if ( ! $counter ) {
				$data['message'] = empty( $message ) ? __( 'The user could not be added. Please contact the website administrator.', 'uncanny-learndash-groups' ) : $message;
				if ( $is_api ) {
					wp_send_json_error( $data );
				} else {
					return $user_id;
				...
				return array(
					'error-code' => empty( $message ) ? __( 'The user could not be added. Check user_email.', 'uncanny-learndash-groups' ) : $message,
					'status'     => array(
						'user'   => 'new_failed',
						// new_created_added_to_group, new_failed, existing_added_to_group, existing_already_member, existing_failed
						'email'  => 'not_sent',
			...
			if ( ! $counter ) {
				...
				return array(
					...
					'status'       => array(
						...
						'email'  => 'not_sent',
						//  sent, not_sent, send_failed
						'reason' => __( 'User has been added.' . $password_info, 'uncanny-learndash-groups' ),
						// '' if success or use_error_code or if needed a custom
					),
				);
			}
		...
		if ( 'yes' === get_option( 'ulgm_send_user_welcome_email', 'yes' ) ) {
			...
			if ( is_wp_error( $welcome_email ) ) {
				...
				if ( ! $counter ) {
					$data['message'] = __( 'User has been added. Welcome email FAILED to send.' . $password_info, 'uncanny-learndash-groups' );
					SharedFunctions::delete_transient( null, $group_id );
					if ( $is_api ) {
						wp_send_json_success( $data );
					}
		...
		if ( ! $counter ) {
			...
			$message = self::user_added_message( 'yes' === get_option( 'ulgm_send_user_welcome_email', 'yes' ) );
		...
		$reason = self::user_added_message( 'yes' === get_option( 'ulgm_send_user_welcome_email', 'yes' ) );
	...
	public static function add_existing_user( $user_data, $do_not_send_emails = false, $group_id = 0, $order_id = 0, $code_status = 'not redeemed', $is_api = true, $is_cron = false, $counter = false, $added_as_gl = false ) {
		...
		// If email exists and username is blank: Make username = email.
		if ( isset( $user_data['user_login'] ) && empty( $user_data['user_login'] ) && empty( $user->user_login ) ) {
			$username_data = array(
				'ID'         => $user_id,
				'user_login' => $user_data['user_email'],
			);

			wp_update_user( $username_data );
		}
		...
		if ( learndash_is_groups_hierarchical_enabled() ) {
			// New hierarchical logic: Only prevent adding if user is directly a member of this specific group
			// Allow adding to child groups even if user is member of parent group
			$is_direct_member  = get_user_meta( $user_id, 'learndash_group_users_' . $group_id, true );
			$is_already_member = $is_direct_member;
		} else {
			// Original logic: Check all user groups to prevent duplicate membership
		...
		if ( $do_not_send_emails ) {
			...
			if ( $counter ) {
				...
				$reason = self::user_added_message( 'yes' === get_option( 'ulgm_send_existing_user_welcome_email', 'yes' ) );
	...
	/**
	 * Bypass any plugin filters that might interfere with password reset during programmatic operations
	 * This method runs with priority 99999 (highest priority, after all other filters)
	 * and returns true to allow password reset, overriding any plugin interference
	 *
	 * @param bool $allow Whether to allow password reset
	 * @param int  $user_id User ID
	 * @return bool Always returns true to bypass plugin interference
	 */
	public static function bypass_password_reset_filters( $allow, $user_id ) {
		// Always return true to override any plugin filters that might interfere
		// during programmatic operations (no form submission)
		return true;
	}
	...
	public static function send_redemption_email( $user_data, $redemption_template_subject, $redemption_template_body, $do_not_send_emails = false, $counter = false ) {
		...
		if ( $user ) {
			...
			if ( learndash_is_groups_hierarchical_enabled() ) {
				// New hierarchical logic: Only prevent adding if user is directly a member of this specific group
				// Allow adding to child groups even if user is member of parent group
				$is_direct_member  = get_user_meta( $user->ID, 'learndash_group_users_' . $group_id, true );
				$is_already_member = $is_direct_member;
			} else {
				// Original logic: Check all user groups to prevent duplicate membership
				$current_user_groups = learndash_get_users_group_ids( $user->ID );
				foreach ( $current_user_groups as $current_user_group ) {
					if ( is_object( $current_user_group ) && $current_user_group->ID === absint( $group_id ) ) {
						$is_already_member = true;
						break;
	 ...
	 * @return string
	 */
	public static function user_added_message( $email_enabled ) {
		$added_email_sent = __( 'User has been added and welcome email is sent.', 'uncanny-learndash-groups' );
		$added            = __( 'User has been added.', 'uncanny-learndash-groups' );

		if ( $email_enabled ) {
			return $added_email_sent;
		}
