...
class WoocommerceLicenseSubscription {
	...
	public function process_subscription_completed( \WC_Subscription $subscription ) {
		...
		$group_created        = $order->get_meta( '_group_already_created', true );
		...
		if ( ! empty( $old_subscription ) && $maybe_resubscription ) {
			if ( is_array( $old_subscription ) ) {
				foreach ( $old_subscription as $o_sub_id => $o_sub_data ) {
					...
					$order->update_meta_data( SharedFunctions::$linked_group_id_meta, $existing_group_id );
					$order->update_meta_data( '_group_already_created', 'yes' );
					$order->update_meta_data( SharedFunctions::$code_group_id_meta_key, $existing_code_group_id );
	...
	public function woo_custom_add_to_cart( $cart_item_data, $product_id, $variation_id ) {
		...
		//global $woocommerce;
		if ( $product instanceof \WC_Product && class_exists( 'WC_Subscriptions_Product' ) && \WC_Subscriptions_Product::is_subscription( $product->get_id() ) ) {
