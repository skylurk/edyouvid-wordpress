...
class WoocommercePaymentComplete {
	...
	public function has_subscription_license_type_product( \WC_Order $order ) {
		...
		foreach ( $items as $item ) {
			...
			if ( $this->is_subscription_type_product( $product ) ) {
				return true;
			}
		}
	...
	/**
	 * Checks if the given product is a subscription type product with linked courses.
	 *
	 * @param \WC_Product $product Product object. Must be an instance of \WC_Product.
	 * @return bool True if product is a subscription type with courses, false otherwise.
	 */
	public function is_subscription_type_product( $product ) {
