...
class WoocommerceMinMaxQuantity {
	...
	public function __construct() {
		...
		add_filter(
			...
			array(
				$this,
				'woocommerce_cart_item_price_refresh',
			),
			10,
			2
		);
	...
	* Get the total quantity of the product available in the cart.
	...
	/**
	 * WooCommerce Cart product item price refresh
	 *
	 * @param $cart_object
	 *
	 * @return string
	 */
	public function woocommerce_cart_item_price_refresh( $price, $cart_item ) {
		$product = wc_get_product( $cart_item['product_id'] );
		if ( ! ( $product instanceof \WC_Product ) ) {
			return $price;
		}
		if ( ! in_array( (string) $product->get_type(), array( 'license', 'subscription' ), true ) ) {
			return $price;
		}
		$fixed_price          = (string) get_post_meta( $cart_item['product_id'], 'ulgm_license_fixed_price', true );
		$_subscription_period = (string) get_post_meta( $cart_item['product_id'], '_subscription_period', true );
		$base_price_string    = get_woocommerce_currency_symbol() . floatval( get_post_meta( $cart_item['product_id'], '_price', true ) ) . ' / ' . $_subscription_period;
		if ( isset( $cart_item['product_id'] ) && 'yes' === $fixed_price ) {
			return $base_price_string . ' / ' . SharedFunctions::get_per_seat_text( 2 );
		}
		return $base_price_string;
	}
}
