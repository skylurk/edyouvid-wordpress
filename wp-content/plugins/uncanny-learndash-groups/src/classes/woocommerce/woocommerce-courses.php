	...
	public function remove_course( $order_id ) {
		...
		foreach ( $products as $product ) {
			...
			if ( $courses_id ) {
				...
				$this->remove_course_access( $courses_id, $order->get_user_id() );
				//}
			}
		}
	}
	...
	public function delete_course_access( $order ) {
		...
		foreach ( $products as $product ) {
			...
			if ( $courses_id ) {
				$course_id = $courses_id;
				//foreach ( $courses_id as $course_id ) {
				$this->remove_course_access( $course_id, $order->get_user_id() );
				//}
			}
		}
	}
	...
	public function give_course_access( $order ) {
		...
		foreach ( $products as $product ) {
			...
			if ( $courses_id ) {
				//foreach ( $courses_id as $course_id ) {
				$course_id = $courses_id;
				if ( empty( $order->get_user_id() ) || empty( $course_id ) ) {
					return;
				}
				$this->add_course_access( $course_id, $order->get_user_id() );
				update_user_meta( $order->get_user_id(), 'course_' . $course_id . '_access_from', strtotime( $start_date ) );
				//}
			}
		}
	}
