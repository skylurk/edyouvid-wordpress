...
class WoocommerceBuyCourses {
	...
	public function plugins_loaded() {
		...
		add_action( 'woocommerce_add_to_cart', array( $this, 'handle_license_product_on_add_to_cart' ), 1099, 6 );
	}
	...
	public function create_custom_product() {
		if ( ! ulgm_filter_has_var( '_custom_buy_courses', INPUT_POST ) ) {
			return;
		}
		if ( ! wp_verify_nonce( ulgm_filter_input( '_custom_buy_courses', INPUT_POST ), Utilities::get_plugin_name() ) ) {
			return;
		}
		if ( ulgm_filter_has_var( 'modify_license', INPUT_POST ) && 'yes' === ulgm_filter_input( 'modify_license', INPUT_POST ) ) {
			$this->edit_custom_product();
	 ...
	 * @return void
	 */
	public function edit_custom_product() {
	...
	public function modify_existing_license( $order_id ) {
		...
		$transient = maybe_unserialize( $order->get_meta( '_ulgm_user_buy_courses_' . $user_id . '_order', true ) );
		...
		if ( 'yes' === (string) get_post_meta( $product_id, '_uo_custom_buy_product', true ) ) {
			...
			if ( $new_courses ) {
				...
				foreach ( $new_courses as $n_c ) {
					...
					if ( ! in_array( $c_id, $existing_c ) ) { // phpcs:ignore
			...
			$order->update_meta_data(
				'order_type',
				sprintf(
					__( '%1$s %2$s(s) added to the group.', 'uncanny-learndash-groups' ),
					join( ', ', $new_course_titles ),
					strtolower(
	...
	/**
	 * Action hook for handling license type products when a new product is added to the cart.
	 *
	 ...
	 */
	public function handle_license_product_on_add_to_cart( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
		...
		// check if there's an existing cart mods
		if ( ! WoocommerceModifyGroup::is_modifying_license() ) {
			return;
		}

		$modifying_product = WoocommerceModifyGroup::get_modifying_license_product_id();

		foreach ( WC()->cart->get_cart() as $_cart_item_key => $cart_item ) {
			if ( $_cart_item_key === $cart_item_key ) {
				continue;
			}

			$cart_product_id = $cart_item['product_id'];

			if ( absint( $modifying_product ) === $cart_product_id ) {
				...
				wc_add_notice(
					esc_html__( 'You cannot combine a new group purchase with a purchase that modifies an existing group. Products linked to the existing group have been removed from your cart.', 'uncanny-learndash-groups' ),
					'error'
