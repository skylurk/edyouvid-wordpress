...
class GroupEssays {
	...
	public function create_essay_table( $request = array() ) {
		...
		var ulgmEssayReportShortcode = {
			...
			loadOnRender: '<?php echo 'no' === strtolower( trim( $request['load_on_render'] ) ) ? 'no' : 'yes'; ?>'
		}
							...
							<div class="uo-groups-select-filters">
								<?php if ( isset( $group_drop_downs['groups'] ) ) { ?>
									<div class="uo-row uo-groups-select-filter">
										<div class="uo-select">
											<label for="uo-group-report-group"><?php _e( 'Group', 'uncanny-learndash-groups' ); ?></label>
								...
								<?php if ( isset( $group_drop_downs['courses'] ) ) { ?>
									<div class="uo-row uo-groups-select-filter">
										<div class="uo-select">
											<label for="uo-group-report-courses"><?php echo \LearnDash_Custom_Label::get_label( 'course' ); ?></label>
											...
											<div id="uo-group-report-nocourses" class="group-management-rest-message"
												style="display: none;"><?php printf( __( 'No %s found.', 'uncanny-learndash-groups' ), \LearnDash_Custom_Label::get_label( 'courses' ) ); ?></div>
										</div>
									</div>
								<?php } ?>
								<?php if ( isset( $group_drop_downs['lessons'] ) ) { ?>
									<div class="uo-row uo-groups-select-filter">
										<div class="uo-select">
											<label for="uo-group-report-lessons"><?php printf( _x( '%1$s / %2$s', 'LearnDash lesson and topic labels', 'uncanny-learndash-groups' ), \LearnDash_Custom_Label::get_label( 'lesson' ), \LearnDash_Custom_Label::get_label( 'topic' ) ); ?></label>
											...
											<div id="uo-group-report-nolessons" class="group-management-rest-message"
												style="display: none;"><?php printf( _x( 'No %s found.', 'No lessons found.', 'uncanny-learndash-groups' ), \LearnDash_Custom_Label::get_label( 'lessons' ) ); ?></div>
										</div>
									</div>
								<?php } ?>
								<?php if ( isset( $group_drop_downs['quizzes'] ) ) { ?>
									<div class="uo-row uo-groups-select-filter">
										<div class="uo-select">
											<label for="uo-group-report-quizzes"><?php echo \LearnDash_Custom_Label::get_label( 'quiz' ); ?></label>
											...
											<div id="uo-group-report-noquizzes" class="group-management-rest-message"
												style="display: none;"><?php printf( _x( 'No %s found.', 'No quizzes found', 'uncanny-learndash-groups' ), \LearnDash_Custom_Label::get_label( 'quizzes' ) ); ?></div>
										</div>
									</div>
								<?php } ?>
								<div class="uo-row uo-groups-select-filter">
									<div class="uo-select">
										<label for="uo-group-report-status"><?php _e( 'Status', 'uncanny-learndash-groups' ); ?></label>
	...
	public function get_groups_drop_downs( $user_id = 0 ) {
		...
		if ( $group_post_objects ) {
			foreach ( $group_post_objects as $group_post_object ) {
				...
				$drop_down['lesson_quizzes'][ $group_post_id ]        = $group_quizzes['lesson_quizzes'];
				$drop_down['course_quizzes'][ $group_post_id ]        = $group_quizzes['course_quizzes'];
				$drop_down['omitted_topic_quizzes'][ $group_post_id ] = $group_quizzes['omitted_topic_quizzes'];
			}
	...
	public function group_quizzes( $group_id = 0 ) {
		...
		$include_topics       = true;
		$relation             = array();
		$lesson_relations     = array();
		$omited_topic_quizzes = array();
			...
			if ( ! empty( $group_course_ids ) ) {
				foreach ( $group_course_ids as $course_id ) {
					...
					if ( ! isset( $relation[ $course_id ] ) ) {
						...
						$lesson_relations[ $course_id ] = array();
					}
							...
							if ( $include_topics ) {
								...
								if ( ! empty( $topic_ids ) ) {
									...
									foreach ( $topic_ids as $topic_id ) {
										...
										if ( ! empty( $quiz_ids ) ) {
											$group_quiz_ids = array_merge( $group_quiz_ids, $quiz_ids );
											// Add to lesson relations
											if ( ! isset( $lesson_relations[ $course_id ][ $lesson_id ] ) ) {
												$lesson_relations[ $course_id ][ $lesson_id ] = array();
											}
											$lesson_relations[ $course_id ][ $lesson_id ] = array_merge( $lesson_relations[ $course_id ][ $lesson_id ], $quiz_ids );
											// Add to topics
											$lesson_relations[ $course_id ][ $topic_id ] = $quiz_ids;
											// Add to omitted topic quizzes
											if ( $omit_topic_quizzes ) {
												$omited_topic_quizzes[ $course_id . '-' . $lesson_id ] = array_merge( $omited_topic_quizzes[ $course_id . '-' . $lesson_id ], $quiz_ids );
											}
										}
									}
								}
		...
		$data = array(
			...
			'relationships'         => $relationships,
			'lesson_quizzes'        => $lesson_relations,
			'course_quizzes'        => $relation,
			'omitted_topic_quizzes' => $omited_topic_quizzes,
		);
	...
	public function essays_scripts( $request = array() ) {
		...
		if ( Utilities::has_shortcode( $post, 'uo_groups_essays' ) || Utilities::has_block( $post, 'uncanny-learndash-groups/uo-groups-essays-report' ) ) {
			...
			$columns_visibilities = array(
				...
				'student'        => in_array( __( 'Student', 'uncanny-learndash-groups' ), $columns ) ? true : false,
			...
			$api_setup = array(
				...
				'lesson_quizzes'        => isset( $group_drop_downs['lesson_quizzes'] ) ? $group_drop_downs['lesson_quizzes'] : array(),
				'course_quizzes'        => isset( $group_drop_downs['course_quizzes'] ) ? $group_drop_downs['course_quizzes'] : array(),
				'omitted_topic_quizzes' => isset( $group_drop_downs['omitted_topic_quizzes'] ) ? $group_drop_downs['omitted_topic_quizzes'] : array(),
				...
				'i18n'                  => array(
					...
					'invalid_points_input' => __( 'Invalid entry. The points awarded cannot exceed the points available.', 'uncanny-learndash-groups' ),
				),
			);
	...
	private function get_frontend_localized_strings() {
		...
		$localized_strings['student'] = __( 'Student', 'uncanny-learndash-groups' );
	...
	public function essays_table( $lesson_ID = 0, $course_ID = 0, $group_ID = 0, $quiz_ID = 0, $status = 'ungraded' ) {
		...
		if ( learndash_is_group_leader_user( $user_id ) || learndash_is_admin_user( $user_id ) ) {
			...
			if ( ! empty( $group_ids ) && is_array( $group_ids ) ) {
				...
				$selected_group_id = absint( $group_ID );
				if ( ! empty( $selected_group_id ) ) {
					$selected_group_ids = array();
					if ( in_array( $selected_group_id, $group_ids, true ) ) {
						// Set the selected group as the only group to loop.
						$selected_group_ids[] = $selected_group_id;
						// Check if it has child groups.
						if ( SharedFunctions::is_hierarchy_enabled( $selected_group_id ) ) {
							$group_children_ids = learndash_get_group_children( $selected_group_id );
							if ( ! empty( $group_children_ids ) && is_array( $group_children_ids ) ) {
								// Filter out the child groups that are not in the group_ids array.
								$group_children_ids = array_intersect( $group_children_ids, $group_ids );
								// Merge the child groups with the selected group.
								$selected_group_ids = array_merge( $selected_group_ids, $group_children_ids );
							}
						}
					}
			...
			if ( ! empty( $course_ids ) && count( $course_ids ) ) {
				...
				if ( ! empty( $lesson_ids ) && count( $lesson_ids ) && $lesson_ID !== '' && $lesson_ID !== 0 && in_array( $lesson_ID, $lesson_ids ) ) {
					...
					// Include lesson topic step IDs
					if ( 'sfwd-lessons' === get_post_type( $lesson_ID ) ) {
						$lesson_course_id = learndash_get_course_id( $lesson_ID );
						if ( true === apply_filters( 'ulgm_rest_api_should_include_lesson_topic_essays_data', true, $lesson_ID, $lesson_course_id ) ) {
							$topic_ids = learndash_course_get_children_of_step( $lesson_course_id, $lesson_ID, 'sfwd-topic' );
							if ( ! empty( $topic_ids ) ) {
								$lesson_ids = array_unique( array_merge( $lesson_ids, $topic_ids ) );
							}
						}
					}
		...
		if ( ! empty( $essay_posts ) ) {
			foreach ( $essay_posts as $essay ) {
				...
				if ( ! empty( $quiz_id ) ) {
					...
					if ( $question instanceof \WpProQuiz_Model_Question ) {
						...
						if ( $essay->post_status == 'not_graded' ) {
							...
							$points = sprintf( esc_html_x( '%1$d / %2$.2f', 'placeholders: current awarded points / maximum point for essay', 'uncanny-learndash-groups' ), $current_points, $max_points );
						}
					} else {
						$points = '-';
					}
				...
				$upload        = $this->get_download_link( $essay_id );
		...
		if ( ! empty( $essay ) ) {
			...
			<div class="group-management-modal group-essay-modal modal">
				<form id="edit_essay_form" method="post">
					...
					<div class="uo-groups">
						<div class="group-management-form">
							<div class="group-management-modal-message" id="group-management-message"></div>
							...
							if ( empty( $essay_quiz_post_id ) ) {
								...
								if ( count( $essay_quiz_query->posts ) > 1 ) {
									...
									<div class="uo-row">
										<div class="uo-row__title">
											<?php printf( esc_html_x( 'Essay %s', 'Essay Quiz', 'uncanny-learndash-groups' ), LearnDash_Custom_Label::get_label( 'quiz' ) ); ?>
										</div>

										<select name="essay_quiz_post_id">
											<option value=""><?php printf( esc_html_x( 'No %s', 'No Quiz', 'uncanny-learndash-groups' ), LearnDash_Custom_Label::get_label( 'quiz' ) ); ?></option>
											<?php
							...
							if ( ! empty( $essay_quiz_post_id ) ) {
								...
								if ( ! empty( $essay_quiz_course_id ) ) {
									...
									<div class="uo-row">
										<div class="uo-row__title">
											<?php printf( esc_html_x( 'Essay %s', 'Essay Course', 'uncanny-learndash-groups' ), \LearnDash_Custom_Label::get_label( 'course' ) ); ?>
										</div>
									...
									if ( ! empty( $essay_quiz_lesson_id ) ) {
										...
										<div class="uo-row">
											<div class="uo-row__title">
												<?php printf( esc_html_x( 'Essay %s', 'Essay Lesson', 'uncanny-learndash-groups' ), \LearnDash_Custom_Label::get_label( 'lesson' ) ); ?>
											</div>
	...
	public function save_essays() {
		...
		if ( ! isset( $post_data['essay_points'] ) ) {
			...
			if ( null !== $max_points && (float) $submitted_essay['points_awarded'] > (float) $max_points ) {
				return array(
					'success' => false,
					'message' => __( 'Invalid entry. The points awarded cannot exceed the points available.', 'uncanny-learndash-groups' ),
				);
			}
	...
	/**
	 * Get essay download link
	 *
	 * @param $essay_id
	 *
	 * @return string
	 */
	public function get_download_link( $essay_id ) {

		if ( function_exists( 'learndash_quiz_essay_get_download_url' ) ) {
			$download_link = learndash_quiz_essay_get_download_url( $essay_id );
		} else {
			// Fallback for older versions of LearnDash below 4.10.3
