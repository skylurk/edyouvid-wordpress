...
class GroupReportsInterface {
	...
	public static function enqueue_frontend_assets() {
		...
		if ( ! empty( $post ) ) {
			...
			$api_setup = array(
				...
				'tableColumns' => self::get_column_translations(),
			);
	...
	public function uo_group_course_report( $attributes, $content = '' ) {
		...
		$attributes = Utilities::fix_params_key( $attributes, 'csv-export-button', 'csv_export_button' );
		$attributes = Utilities::fix_params_key( $attributes, 'excel-export-button', 'excel_export_button' );
		$request    = shortcode_atts(
			array(
				'transcript-page-id'  => 0,
				'course-order'        => '',
				'columns'             => 'user_name,first_name,last_name,user_email,percent_completed,date_completed,date_enrolled,course_name,group_name,transcript_page_url,course_time',
				'orderby_column'      => esc_attr__( 'Date Completed', 'uncanny-learndash-groups' ),
		...
		if ( isset( $request['columns'] ) && ! empty( $request['columns'] ) ) {
			...
			// Get any columns added via filters.
			$filtered_columns = self::get_column_translations();
			// Merge the columns that are allowed with the filtered columns.
			$column_check = array_keys( array_diff_key( $filtered_columns, array_keys( $allowed_columns ) ) );

			if ( ! empty( $columns ) ) {
				foreach ( $columns as $column ) {
					if ( in_array( (string) $column, $column_check, true ) ) {
		...
		$js_extra_inline_data = array(
			...
			'showCSVExportButton'       => 'show' === strtolower( trim( $request['csv_export_button'] ) ),
			'showExcelExportButton'     => 'show' === strtolower( trim( $request['excel_export_button'] ) ),
		);
	...
	public function get_user_course_data() {
		...
		if ( 'get-user-data-courses' === $action ) {
			...
			if ( $group_id && current_user_can( $permission ) && ! current_user_can( 'manage_options' ) ) {
				$groups_administrator_ids = learndash_get_groups_administrator_ids( $group_id );
				if ( ! in_array( get_current_user_id(), $groups_administrator_ids ) ) {
					wp_send_json_error( __( 'Only Leaders of the associated group can view the group.', 'uncanny-learndash-groups' ) );
				}
			}
	...
	public static function user_course_data( $course_id, $group_id, $user_id = false ) {
		...
		// Check if hierarchial groups are enabled.
		if ( empty( $parent_enrolled ) ) {
			if ( SharedFunctions::is_hierarchy_enabled( $group_id ) ) {
				// Find the child group that the course is enrolled to.
				$children_group_ids = learndash_get_group_children( $group_id );
				if ( ! empty( $children_group_ids ) ) {
					foreach ( $children_group_ids as $child_group_id ) {
						$child_enrolled = get_post_meta( $course_id, "learndash_group_enrolled_{$child_group_id}", true );
						if ( ! empty( $child_enrolled ) ) {
							// Update the group title to the child group title.
							$group_title = get_the_title( $child_group_id );
							break;
						}
					}
				}
	 ...
	 * @return string|null
	 */
	public static function get_course_time_in_seconds( $course_id, $user_ID, $raw = false ) {
