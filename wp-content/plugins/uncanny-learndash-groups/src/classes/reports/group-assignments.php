...
class GroupAssignments {
						...
						<div class="group-management-form">
							<div class="uo-groups-select-filters">
								...
								<?php if ( isset( $group_drop_downs['courses'] ) ) { ?>
									<div class="uo-row uo-groups-select-filter">
										<div class="uo-select">
											<label for="uo-group-report-courses"><?php _e( \LearnDash_Custom_Label::get_label( 'course' ), 'uncanny-learndash-groups' ); ?></label>
						...
						if (2 === coursesSelectOptions.length) {
							...
							if (typeof groupslessons !== 'undefined' && groupslessons.length > 0) {
								...
								setTimeout(function () {
									...
									jQuery('#uo-group-report-lessons').prop('disabled', false);
									jQuery('#uo-group-report-lessons').removeAttr('disabled');
								}, 200);
							} else {
								jQuery('#uo-group-report-lessons option').hide();
								jQuery('#uo-group-report-lessons option[value=' + 0 + ']').show();
	...
	public function assignments_table( $lesson_ID = 0, $course_ID = 0, $group_ID = 0, $_status = 'not-approved' ) {
		...
		if ( ! empty( $assignment_posts ) ) {
			foreach ( $assignment_posts as $a_post ) {
				...
				$download_link = $this->get_download_link( $assignment_id );
				...
				$date_col  = '<span class="ulg-hidden-data" style="display: none;">' . $timestamp . '</span>' . $date;
				...
				<form id="edit_assignment_form" method="post">
					...
					<div class="uo-groups">
						<div class="group-management-form">
							...
							$file_link = $this->get_download_link( $assignment_id );
							...
							<div class="uo-row">
								<div class="uo-row__title">
									<?php printf( esc_html_x( 'Associated %s', 'Associated Course Label', 'uncanny-learndash-groups' ), \LearnDash_Custom_Label::get_label( 'course' ) ); ?>
								</div>
								...
								if ( empty( $assignment_course_id ) ) {
									...
									<select name="sfwd-assignment_course">
										<option value=""><?php printf( esc_html_x( '-- Select a %s --', 'Select a Course Label', 'uncanny-learndash-groups' ), \LearnDash_Custom_Label::get_label( 'course' ) ); ?></option>
										<?php
										$cb_courses = array();
										if ( ! empty( $assignment_lesson_id ) ) {
							...
							<div class="uo-row">
								<div class="uo-row__title">
									<?php printf( esc_html_x( 'Associated %s', 'Associated Lesson Label', 'uncanny-learndash-groups' ), \LearnDash_Custom_Label::get_label( 'lesson' ) ); ?>
								</div>
								...
								if ( empty( $assignment_lesson_id ) ) {
									...
									<select name="sfwd-assignment_lesson">
										<option value=""><?php printf( esc_html_x( '-- Select a %s --', 'Select a Lesson Label', 'uncanny-learndash-groups' ), \LearnDash_Custom_Label::get_label( 'lesson' ) ); ?></option>
										<?php
										if ( ! empty( $assignment_course_id ) ) {
	...
	/**
	 * Get assignment download link
	 *
	 * @param $assignment_id
	 *
	 * @return string
	 */
	public function get_download_link( $assignment_id ) {

		if ( function_exists( 'learndash_assignment_get_download_url' ) ) {
			$download_link = learndash_assignment_get_download_url( $assignment_id );
		} else {
