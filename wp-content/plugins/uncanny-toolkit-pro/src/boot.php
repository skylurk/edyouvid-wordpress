...
class Boot extends toolkit\Config {
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			require_once __DIR__ . '/classes/class-uo-metabox-tab.php';

			// Authoritative active-module list (same source the module loop above uses).
			$ultp_active = array_map(
				static function ( $c ) {
					return str_replace( '\\', '', (string) $c );
				},
				(array) self::get_active_classes()
			);
			$ultp_is_active = static function ( $class ) use ( $ultp_active ) {
				return in_array( str_replace( '\\', '', $class ), $ultp_active, true );
			};

			$ultp_tab_label = __( 'Uncanny Toolkit', 'uncanny-pro-toolkit' );
			$ultp_config    = array();

			// Groups: login redirect + expiration boxes.
			$ultp_group_boxes = array();
			if ( $ultp_is_active( 'uncanny_pro_toolkit\GroupLoginRedirect' ) ) {
				$ultp_group_boxes[] = 'uo-toolkit-group-redirect';
			}
			if ( $ultp_is_active( 'uncanny_pro_toolkit\learnDashGroupExpiration' ) ) {
				$ultp_group_boxes[] = 'expiration_date';
				// expiration_email registers only when the email setting is on; harmless if
				// absent (LD's tab JS getElementById null-checks).
				$ultp_group_boxes[] = 'expiration_email';
			}
			if ( ! empty( $ultp_group_boxes ) ) {
				$ultp_config['groups'] = array(
					'tab_id'    => 'uo-toolkit-groups',
					'tab_label' => $ultp_tab_label,
					'metaboxes' => $ultp_group_boxes,
				);
			}

			// Courses: short description.
			if ( $ultp_is_active( 'uncanny_pro_toolkit\ShowAllCourses' ) ) {
				$ultp_config['sfwd-courses'] = array(
					'tab_id'    => 'uo-toolkit-courses',
					'tab_label' => $ultp_tab_label,
					'metaboxes' => array( 'ultp-course-short-description' ),
				);
			}

			// Lessons: group drip settings (LD-native settings metabox).
			if ( $ultp_is_active( 'uncanny_pro_toolkit\UncannyDripLessonsByGroup' ) ) {
				$ultp_config['sfwd-lessons'] = array(
					'tab_id'    => 'uo-toolkit-lessons',
					'tab_label' => $ultp_tab_label,
					'metaboxes' => array( 'learndash-lesson-group-drip-settings' ),
				);
			}

			// Topics: group drip settings (LD-native settings metabox).
			if ( $ultp_is_active( 'uncanny_pro_toolkit\UncannyDripTopicsByGroup' ) ) {
				$ultp_config['sfwd-topic'] = array(
					'tab_id'    => 'uo-toolkit-topics',
					'tab_label' => $ultp_tab_label,
					'metaboxes' => array( 'learndash-topic-group-drip-settings' ),
				);
			}

			UO_Metabox_Tab::boot( $ultp_config );
		}
	...
	public static function licensing_call( $endpoint = 'check-license', $force = false ) {
		...
		if ( false !== $license_data && false === $force && isset( $license_data->license_html ) ) {
			return $license_data;
		}
