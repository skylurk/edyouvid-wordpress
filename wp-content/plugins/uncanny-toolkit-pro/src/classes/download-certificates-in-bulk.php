...
This link will expire in ' . date_i18n( 'H', self::uo_file_expiry_time() ) . ' hours.',
			...
			array(
				'type'       => 'html',
				'inner_html' => '<a class="uncannyowl-btn uncannyowl-btn--secondary uncannyowl-btn--sm uo_certificates_cleanup_btn" href="javascript:void(0)">Delete certificate zip files from the server</a>',
			),
			array(
				'type'       => 'html',
			...
			array(
				'type'       => 'html',
				'inner_html' => '<a class="uncannyowl-btn uncannyowl-btn--secondary uncannyowl-btn--sm uo_certificates_clear_btn" href="javascript:void(0)">Clear previous runs</a>',
			),
			array(
				'type'       => 'html',
					...
					<div id="uo_download_certificates">
						<div class="uo-grid-wrapper" id="uo_certificatesdropdown">
							...
							<div
								...
								<button
									id="uo_generate_certificates_submit" class="uncannyowl-btn uncannyowl-btn--primary"><?php esc_html_e( 'Generate certificates', 'uncanny-pro-toolkit' ); ?></button>
								<?php 
	...
	public static function uo_send_email_for_empty_files( $user, $batch_id ) {
		...
		$email_message         = __( 'Hi %User First Name%,<br /><br />The recent attempt to generate %Certificates for% certificates at %Time requested% resulted in no matches, so a zip file was not created.<br />Please change your certificate filters and try again.', 'uncanny-pro-toolkit' ); // phpcs:ignore
