...
class FluentformLessonTopicAutoComplete extends toolkit\Config implements toolkit\RequiredFunctions {
	...
	public static function create_hidden_field_ff( $form ) {
		...
		$current_post = $post instanceof \WP_Post ? $post : get_post( get_queried_object_id() );
		...
		if ( 'sfwd-lessons' === $current_post->post_type ) {
			...
			$lesson_id = $current_post->ID;
		}
		if ( 'sfwd-topic' === $current_post->post_type ) {
			$course_id = learndash_get_course_id( $current_post->ID );
			$topic_id  = $current_post->ID;
		}
	...
	public static function remove_mark_complete_button_focus( $return, $post ) {
		...
		if ( function_exists( 'has_blocks' ) ) {
			if ( has_blocks( $post->post_content ) ) {
				...
				foreach ( $blocks as $block ) {
					if ( $block['blockName'] === 'fluentform/guten-block' ) {
		...
		if ( has_shortcode( $post->post_content, 'fluentform' ) ) {
			...
			if ( ! isset( $short_codes['fluentform'] ) && $form_id == 0 ) {
				return $return;
			}
	...
	public static function learndash_show_next_link_progression( $show_next_link = false, $user_id = 0, $post_id = 0 ) {
		...
		if ( ! has_shortcode( $module->post_content, 'fluentform' ) || ! $lesson_progression_enabled ) {
			return $show_next_link;
		}

		// Do not hide next lesson if fluentform is ajax submission
		$pattern = get_shortcode_regex();
		if ( preg_match_all( '/' . $pattern . '/s', $module->post_content, $matches ) && array_key_exists( 2, $matches ) && in_array( 'fluentform', $matches[2] ) ) {
			$all_matched = $matches[0]; // contains all shortcodes in content
			if ( $all_matched ) {
				foreach ( $all_matched as $m ) {
					//Match wpform with ajax="true"
					if ( preg_match( '/(\[fluentform(.*)(ajax)(.*)\])/', $m ) ) {
						return $show_next_link;
					}
				}
			}
	...
	public static function get_lead_id( $user_id, $form_id ) {
		...
		if ( $wpdb->get_var( $query ) !== $entries_table ) {
			...
			$entry = $wpdb->get_row( $wpdb->prepare( "SELECT id FROM {$entries_table} WHERE user_id = %d AND form_id = %d", $user_id, $form_id ) );
