...
class LearnDashGroupSignUp extends toolkit\Config implements toolkit\RequiredFunctions {
	...
	public function __construct() {
		...
		if ( wp_doing_ajax() ) {
			...
			// Fires when user is registered with Forminator Form.
			add_action( 'forminator_cform_user_registered', array( $this, 'add_to_group_on_ajax_forminator_user_registered' ), 99, 4 );
		}
	}
	...
	public static function run_frontend_hooks() {
		if ( true === self::dependants_exist() ) {
			...
			add_action( 'uo_group_signup_render_groups_register_form', array( __CLASS__, 'render_groups_register_form'), 20, 2);
		}
	}
	...
	public static function add_to_group_on_user_registered( $user_id ) {
		if ( isset( $_REQUEST['gid'] ) && absint( $_REQUEST['gid'] ) ) {
			// Set the in-memory user context before group assignment so that
			// hooks fired by set_users_group (e.g. ld_added_group_access)
			// see a valid current user for plugins like Automator.
			//
			// Only wp_set_current_user() is called here — it is in-memory
			// only with no session side-effects. This is intentionally called
			// for all registration paths (including GF and TML). GF/TML set
			// their own user context later in their auth flows, which simply
			// overwrites this in-memory value. The auth cookie is set later,
			// guarded by the GF/TML check to avoid interfering with their
			// own session handling (see Ticket #9720).
			//
			// When FrontendLoginPlus verification is enabled, we skip this
			// because the user must complete email verification before being
			// treated as authenticated — setting current user here would
			// bypass that verification intent.
			...
			// GF and TML handle their own auth; only set cookie and redirect
			// for other registration paths (see Ticket #9720).
	...
	/**
	 * Adds user to group when registered via Forminator form with AJAX.
	 *
	 * @param int    $user_id     The newly registered user ID.
	 * @param object $custom_form The Forminator form object.
	 * @param array  $entry       The form entry data.
	 * @param string $user_pass   The user password.
	 *
	 * @return void
	 */
	public function add_to_group_on_ajax_forminator_user_registered( $user_id, $custom_form, $entry, $user_pass ) {
		// Check the referer URL from _wp_http_referer field in POST data
		$referer_url = isset( $_POST['_wp_http_referer'] ) ? esc_url_raw( wp_unslash( $_POST['_wp_http_referer'] ) ) : '';

		// If the referer URL is empty, log and return
		if ( empty($referer_url) ) {
			return;
		}

		// Extract query parameters from the referer URL
		$query_params = $this->extract_query_params($referer_url);

		// Check if the 'gid' parameter exists
		if ( empty( $query_params['gid'] ) ) {
			error_log( sprintf( 
				'Forminator group assignment failed: gid not found in referer URL. User ID: %d, Referer: %s',
				$user_id,
				$referer_url
			) );
			return;  // Exit if 'gid' is not found
		}

		// Now assign the user to the group based on the 'gid'
		self::set_users_group( absint( $query_params['gid'] ), $user_id );
	}
	...
	/**
	 * Renders the group registration form if no supported form is detected.
	 *
	 * @param WP_Post $group_post   The group post object.
	 * @param bool    $show_content Whether to show content.
	 *
	 * @return void
	 */
	public static function render_groups_register_form($group_post, $show_content) {
		if ( empty( $group_post ) || empty( $show_content ) || empty( $group_post->ID ) ) {
			return;
		}

		if ( ! self::has_supported_forms( $group_post->post_content ) && $show_content ) {
			self::groups_register_form();
		}
	}
	 ...
	 * @return bool
	 */
	public static function has_supported_forms( $content ) {
		// Block detection.
		if ( function_exists( 'has_blocks' ) && has_blocks( $content ) ) {
			...
			foreach ( $blocks as $block ) {
				...
				if (
					strpos( $block_name, 'gravityforms/form' ) === 0 ||
    				strpos( $block_name, 'forminator/forms' ) === 0
				) {
					return true;
				}
			}
		...
		// Shortcode detection (explicit).
		return
			has_shortcode( $content, 'gravityform' ) ||
			has_shortcode( $content, 'forminator_form' ) ||
			has_shortcode( $content, 'theme-my-login' );
	}
}
