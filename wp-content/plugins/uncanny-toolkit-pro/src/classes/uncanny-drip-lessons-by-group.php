...
class UncannyDripLessonsByGroup extends toolkit\Config implements toolkit\RequiredFunctions {
	 ...
	 * @var array
	 */
	public static $notification_meta_keys = array(
	...
	public static function run_frontend_hooks() {
		if ( true === self::dependants_exist() ) {
			...
			// Review add version check to see if this is needed?
	...
	public static function unset_notifications( $lesson_id, $course_id = 0, $group_id = 0 ) {
		...
		if ( empty( $course_id ) || 0 === $course_id ) {
			...
			if ( 0 === (int) $course_id && ( isset( $_REQUEST['course_id'] ) ) && ( ! empty( $_REQUEST['course_id'] ) ) ) {
				$course_id = intval( filter_input( INPUT_GET, 'course_id' ) );
			}
		...
		// Per-user cleanup only if the group currently has members. The scheduled-hook
		// cleanup below is keyed by group/course/lesson (not user) and must run either way,
		// otherwise stale crons survive when a group is emptied after scheduling.
	...
	public static function set_notifications( $lesson_id, $course_id = 0, $group_id = 0, $lesson_access_from = '' ) {
		...
		// Per-user cleanup only if the group currently has members. The cron scheduling
		// via manually_set_notification() must run either way — group membership is
		// re-evaluated at fire time in handle_cron(), so scheduling an event for a
		// currently-empty group is correct (users added later will still receive it).
		$users = learndash_get_groups_user_ids( $group_id );
		if ( $users ) {
			foreach ( $users as $user_id ) {
				self::delete_delayed_email_data( $user_id, $group_id, $course_id, $lesson_id );
	...
	public static function get_raw_notifications( $group_id, $course_id, $lesson_id ) {
		...
		foreach ( $notifications as $notification ) {
			...
			if ( self::validate_notification_ids( $notification->ID, $group_id, $course_id, $lesson_id ) ) {
				$return[] = $notification;
			}
		}
	...
	public static function ld_lesson_access_from_func( $access_from, $lesson_id, $user_id ) {
		if ( ! is_admin() || wp_doing_ajax() ) {
			...
			if ( $has_group_access ) {
				...
				if ( ! empty( $group_access ) && __( 'Available', 'uncanny-pro-toolkit' ) === (string) $group_access ) {
					return null;
				}
	...
	public static function save_lesson_custom_meta( $settings_field_updates, $settings_metabox_key, $settings_screen_id ) {
		...
		if ( self::$access_metabox_key === $settings_metabox_key ) {
			...
			if (
				filter_has_var( INPUT_POST, 'learndash-lesson-access-settings' ) &&
				...
				if ( isset( $_POST['learndash-lesson-access-settings']['set_groups_for_dates'] ) ) {
					...
					if ( ! empty( $group_id ) ) {
						...
						$course_id = filter_has_var( INPUT_GET, 'course_id' ) ? absint( filter_input( INPUT_GET, 'course_id' ) ) : learndash_get_course_id( $post->ID );
	...
	public static function change_hooks_and_shortcodes() {
		...
		if( defined('LEARNDASH_ELEMENTOR_VERSION') ){
			add_filter( 'the_content', array( __CLASS__, 'lesson_visible_the_content' ), 99, 1 );
		}
...
//	}
	...
	public static function group_lesson_available_settings( $settings ) {
		...
		// Newer Settings.
		if ( isset( $settings['trigger_objects'] ) ) {
			$settings['trigger_objects']['fields']['group_id']['parent'][]  = self::$trigger_name;
			$settings['trigger_objects']['fields']['course_id']['parent'][] = self::$trigger_name;
			$settings['trigger_objects']['fields']['lesson_id']['parent'][] = self::$trigger_name;
		}
	...
	public static function save_course_id_lesson_id_notifications( $notification_id ) {
		...
		if ( ! filter_has_var( INPUT_POST, 'learndash_notifications_nonce' ) ) {
			return;
		}
		...
		if ( self::$trigger_name === (string) filter_input( INPUT_POST, '_ld_notifications_trigger' ) ) {
			foreach ( self::$notification_meta_keys as $key ) {
				if ( isset( $_POST[ $key ] ) ) {
					$value = $_POST[ $key ];
					// Newer versions.
					if ( is_array( $value ) ) {
						// Clear all if 'all' is selected or convert to int.
						$value = in_array( 'all', $value, true ) ? array( 'all' ) : array_map( 'absint', $value );
					} else {
						// Legacy versions.
						$value = 'all' === sanitize_text_field( $value ) ? 'all' : absint( $value );
					}
					update_post_meta( $notification_id, $key, $value );
				}
			}

			// Re-schedule notifications for all matching groups/courses/lessons with drip dates
			self::reschedule_notifications_for_saved_notification( $notification_id );
		}
	}
	...
	public static function handle_cron( $notification_id, $group_id, $course_id, $lesson_id ) {
		...
		self::notification_log( sprintf( __( 'Cron args: Notification: %d, Group: %d, Course: %d, Lesson: %d', 'uncanny-pro-toolkit' ), $notification_id, $group_id, $course_id, $lesson_id ) );

		if ( ! class_exists( 'LearnDash_Notifications' ) ) {
			self::notification_log( __( '[UO LearnDash_Notifications] LearnDash_Notifications class is missing', 'uncanny-pro-toolkit' ) );
			return;
		}

		$notifications = self::get_notifications( $notification_id, $group_id, $course_id, $lesson_id );
		if ( empty( $notifications ) ) {
			self::notification_log( __( '[UO LearnDash_Notifications] No notifications', 'uncanny-pro-toolkit' ) );
			return;
		}

		$group_users = learndash_get_groups_users( $group_id, true );
		if ( empty( $group_users ) ) {
			self::notification_log( __( '[UO LearnDash_Notifications] No group users', 'uncanny-pro-toolkit' ) );
			return;
		}

		$user_ids = wp_list_pluck( $group_users, 'ID' );
		if ( empty( $user_ids ) ) {
			self::notification_log( __( '[UO LearnDash_Notifications] No group user ids', 'uncanny-pro-toolkit' ) );
			return;
		}
		...
		foreach ( $user_ids as $user_id ) {
			...
			if ( ! self::should_send( $user_id, $lesson_id, $course_id, $group_id ) ) {
				continue;
			}
			...
			self::notification_log(
				sprintf(
					__( 'Expected to send a notification for the lesson %d at %s for the user %d', 'uncanny-pro-toolkit' ),
					$lesson_id,
					is_numeric( $timestamp ) ? date_i18n( 'Y-m-d H:i:s', $timestamp ) : 'N/A',
					$user_id
				)
			);

			if ( is_numeric( $timestamp ) && $timestamp > 0 && $current < $timestamp ) {
				self::notification_log(
					sprintf(
						__( 'Cron was triggered too early. Current time: %s, Cron time: %s', 'uncanny-pro-toolkit' ),
						date_i18n( 'Y-m-d H:i:s', $current ),
			...
			self::notification_log(
				sprintf(
					__( 'Access time reached for user %d. Current: %s, Access: %s', 'uncanny-pro-toolkit' ),
					$user_id,
					date_i18n( 'Y-m-d H:i:s', $current ),
					is_numeric( $timestamp ) ? date_i18n( 'Y-m-d H:i:s', $timestamp ) : 'N/A'
				)
			...
			foreach ( $notifications as $notification ) {
				if ( self::is_sent( $user_id, $notification->ID, $group_id, $course_id, $lesson_id, $timestamp ) ) {
					continue;
				}
		...
		if ( $sent_any ) {
			...
			self::notification_log( __( 'Notifications sent, cron cleared', 'uncanny-pro-toolkit' ) );
		} else {
			self::notification_log( __( 'No notifications sent, cron preserved', 'uncanny-pro-toolkit' ) );
		}
	 ...
	 * @param int $lesson_id
	 */
	public static function validate_notification_ids( $notification_id, $group_id, $course_id, $lesson_id ) {

		$validated = array();
		foreach ( self::$notification_meta_keys as $key ) {
			$meta_value = get_post_meta( $notification_id, $key, true );
			// Force arrays.
			if ( ! is_array( $meta_value ) ) {
				$meta_value = array( $meta_value );
			}
			// Sanitize int in array.
			foreach ( $meta_value as $k => $value ) {
				if ( 'all' !== $value ) {
					$meta_value[ $k ] = absint( $value );
				}
			}

			// Check if all is set.
			$all_allowed = in_array( 'all', $meta_value, true );

			// Check if notification is set for the lesson.
			if ( '_ld_notifications_group_id' === $key ) {
				$validated['group_id'] = $all_allowed || in_array( absint( $group_id ), $meta_value, true ) ? $group_id : 'invalid';
			}
			if ( '_ld_notifications_course_id' === $key ) {
				$validated['course_id'] = $all_allowed || in_array( absint( $course_id ), $meta_value, true ) ? $course_id : 'invalid';
			}
			if ( '_ld_notifications_lesson_id' === $key ) {
				$validated['lesson_id'] = $all_allowed || in_array( absint( $lesson_id ), $meta_value, true ) ? $lesson_id : 'invalid';
			}
		}

		// If any value is invalid, return false.
		if ( in_array( 'invalid', $validated, true ) ) {
			return false;
		}

		// If all values in the array are 'all' then return true.
		if ( count( array_unique( $validated ) ) === 1 && 'all' === $validated['group_id'] ) {
			return true;
		}

		// Check to ensure $course_id belongs to $group_id.
		$group_courses = learndash_get_group_courses_list( $group_id );
		if ( ! in_array( $course_id, $group_courses, true ) ) {
			return false;
		}
		// Check to ensure $lesson_id belongs to $course_id.
		$course_lessons = self::get_course_lesson_ids( $course_id );
	...
	public static function lesson_visible_the_content( $content ) {
		if ( ! is_singular( learndash_get_post_type_slug( 'lesson' ) ) ) {
			return $content;
		}

		global $post;
		return self::lesson_visible_after( $content, $post );
	}

	/**
	 * Re-schedule notifications for all matching groups/courses/lessons with drip dates
	 * when a notification is saved/edited
	 *
	 * This is more efficient - it queries directly for existing drip dates rather than
	 * looping through all possible groups/courses/lessons
	 *
	 * Side effect: matching is gated on the saved notification's IDs, but the actual
	 * (re)scheduling is delegated to manually_set_notification() -> get_raw_notifications(),
	 * which re-fetches ALL active notifications for the same group/course/lesson. So saving
	 * notification A also reschedules notification B's cron when both target the same combo.
	 * This is intentional: it makes a save a recovery path for every notification on that
	 * combo, and the (un)schedule is idempotent.
	 *
	 * @param int $notification_id Notification post ID
	 */
	public static function reschedule_notifications_for_saved_notification( $notification_id ) {
		if ( ! class_exists( 'LearnDash_Notifications' ) ) {
			return;
		}

		// Get notification meta
		$notification_group_ids = get_post_meta( $notification_id, '_ld_notifications_group_id', true );
		$notification_course_ids = get_post_meta( $notification_id, '_ld_notifications_course_id', true );
		$notification_lesson_ids = get_post_meta( $notification_id, '_ld_notifications_lesson_id', true );

		// Normalize to arrays
		if ( ! is_array( $notification_group_ids ) ) {
			$notification_group_ids = array( $notification_group_ids );
		}
		if ( ! is_array( $notification_course_ids ) ) {
			$notification_course_ids = array( $notification_course_ids );
		}
		if ( ! is_array( $notification_lesson_ids ) ) {
			$notification_lesson_ids = array( $notification_lesson_ids );
		}

		// Query database directly for all drip date meta entries (much more efficient)
		global $wpdb;
		$meta_key_pattern = $wpdb->esc_like( stripslashes( __CLASS__ ) . '-' ) . '%';
		$meta_exclude_all = stripslashes( __CLASS__ ) . '-all';
		$drip_meta_entries = $wpdb->get_results( $wpdb->prepare(
			"SELECT post_id, meta_key, meta_value FROM {$wpdb->postmeta}
			WHERE meta_key LIKE %s AND meta_key != %s AND meta_value != '' AND meta_value != '0'",
			$meta_key_pattern,
			$meta_exclude_all
		) );

		if ( empty( $drip_meta_entries ) ) {
			return;
		}

		// Normalize match sets once — they are derived from the fixed notification meta.
		$group_all     = in_array( 'all', $notification_group_ids, true );
		$course_all    = in_array( 'all', $notification_course_ids, true );
		$lesson_all    = in_array( 'all', $notification_lesson_ids, true );
		$group_id_set  = array_map( 'absint', $notification_group_ids );
		$course_id_set = array_map( 'absint', $notification_course_ids );
		$lesson_id_set = array_map( 'absint', $notification_lesson_ids );

		// Cache course lookups per lesson to avoid an N+1 query when several groups
		// share the same lesson across drip entries.
		$lesson_course_cache = array();

		// Process each drip date entry
		foreach ( $drip_meta_entries as $entry ) {
			$lesson_id = absint( $entry->post_id );
			$drip_date = $entry->meta_value;

			// Normalize to timestamp (handles legacy array or string date storage)
			if ( ! self::is_timestamp( $drip_date ) ) {
				$unserialized = maybe_unserialize( $drip_date );
				$drip_date    = is_array( $unserialized ) ? self::reformat_date( $unserialized ) : 0;
			} else {
				$drip_date = (int) $drip_date;
			}

			// Extract group_id from meta_key (format: "UncannyDripLessonsByGroup-{group_id}")
			$meta_key_parts = explode( '-', $entry->meta_key );
			$group_id = absint( end( $meta_key_parts ) );

			if ( empty( $group_id ) || empty( $lesson_id ) || empty( $drip_date ) ) {
				continue;
			}

			// Check if this group matches notification settings
			if ( ! $group_all && ! in_array( $group_id, $group_id_set, true ) ) {
				continue;
			}

			// Get course for this lesson (cached across entries sharing a lesson)
			if ( ! isset( $lesson_course_cache[ $lesson_id ] ) ) {
				$lesson_course_cache[ $lesson_id ] = absint( learndash_get_course_id( $lesson_id ) );
			}
			$course_id = $lesson_course_cache[ $lesson_id ];
			if ( empty( $course_id ) ) {
				continue;
			}

			// Check if course matches notification settings
			if ( ! $course_all && ! in_array( $course_id, $course_id_set, true ) ) {
				continue;
			}

			// Verify course belongs to group
			if ( ! learndash_group_has_course( $group_id, $course_id ) ) {
				continue;
			}

			// Check if lesson matches notification settings
			if ( ! $lesson_all && ! in_array( $lesson_id, $lesson_id_set, true ) ) {
				continue;
			}

			// Validate that this notification matches this group/course/lesson
			if ( self::validate_notification_ids( $notification_id, $group_id, $course_id, $lesson_id ) ) {
				// Re-schedule the notification for this group/course/lesson
				self::manually_set_notification( $group_id, $course_id, $lesson_id, $drip_date );
			}
		}
	}
}
