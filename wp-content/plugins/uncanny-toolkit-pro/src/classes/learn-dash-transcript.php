...
class LearnDashTranscript extends toolkit\Config implements toolkit\RequiredFunctions {
	private static $transcript_salt = 'uo_toolkit_pro_transcript';
	private static $public_transcript_arg = 'tuuid';
	...
	public static function get_class_settings( $class_title ) {
		...
		$options[] = array(
			'type'        => 'checkbox',
			'label'       => esc_html__( 'Allow public transcript access', 'uncanny-pro-toolkit' ),
			'option_name' => 'uncanny-allow-public-transcript-access',
		);
	...
	public static function display_course_transcript( $attributes ) {
		$is_on_shared_url = self::is_public_sharing_url();

		// Sharing URL and Sharing is disabled? - Bailout.
		if( $is_on_shared_url && ! self::is_public_sharing_allowed() ) {
			$html = esc_html__( 'Invalid request.', 'uncanny-pro-toolkit' );

			return $html;
		}

		if ( ! is_user_logged_in() && ! $is_on_shared_url ) {
		...
		$shared_transcript_user_id = 0;
		$current_user = wp_get_current_user();
		// Is user viewing shareable URL?
		if( $is_on_shared_url ) {
			// Get the validated user id from the URL.
			$shared_transcript_user_id = self::get_valid_public_transcript_user();
			if( false === $shared_transcript_user_id ) {
				$html = esc_html__( 'Invalid URL: This transcript URL is invalid.', 'uncanny-pro-toolkit' );

				return $html;
			}

			$user_id = $shared_transcript_user_id;
		}

		if ( $user_id > 0 ) {

			if ( ! $is_on_shared_url && ( absint( $user_id ) !== absint( $current_user->ID ) ) && ! user_can( $current_user->ID, 'group_leader' ) && ! user_can( $current_user->ID, 'manage_options' ) ) {
		...
		if ( empty( trim( $request['user-id'] ) ) ) {
			...
			if ( ! $is_on_shared_url &&  user_can( $current_user->ID, 'group_leader' ) && ! user_can( $current_user->ID, 'manage_options' ) ) {
				if ( ! learndash_is_group_leader_of_user( $current_user->ID, absint( $request['user-id'] ) ) ) {
	...
	private static function generate_transcript( $data, $request, $user ) {
		...
		if ( 'alpha-desc' === $sort_order ) {
			uasort( $transcript->table->rows, function ( $a, $b ) {
...
//		}
	...
	public static function is_public_sharing_url() {
		$transcript_uuid = $_GET[self::$public_transcript_arg] ?? '';

		// Return false if the UUID is empty or public sharing is not allowed
		if ( ! empty( $transcript_uuid ) ) {
			return true;
		}
	...
	/**
	 * Checks and Validates transcript sharing parameter.
	 *
	 * @return bool True if the public sharing link is present, false otherwise.
	 */
	public static function is_valid_public_sharing_link() {
		// Retrieve the transcript UUID from the URL
		$transcript_uuid = $_GET[self::$public_transcript_arg] ?? '';
		...
		$transcript_uuid_params = self::get_public_sharing_params( $transcript_uuid );

		// Return false if the UUID parameters are not valid
		if (false === $transcript_uuid_params) {
			return false;
		}
	...
	/**
	 * Retrieves and validates the public sharing parameters from the transcript UUID.
	 *
	 * @param string $transcript_uuid The transcript UUID.
	 * @return array|false Array of UUID parameters if valid, false otherwise.
	 */
	public static function get_public_sharing_params($transcript_uuid) {
        // Return false if the UUID is empty, not hexadecimal, or not 96 characters long
        if ( empty($transcript_uuid) || ! ctype_xdigit($transcript_uuid) || strlen($transcript_uuid) !== 96 ) {
            return false;
        }

        // Split the UUID into parts
        $transcript_uuid_params = str_split($transcript_uuid, 32);
        if ( 3 !== count( $transcript_uuid_params) ) {
            return false;
        }

        // Validate the salted hash
        $salted_hash = md5(self::$transcript_salt);
        if ( $transcript_uuid_params[0] !== $salted_hash ) {
            return false;
        }

        return $transcript_uuid_params;
    }

	/**
	 * Checks if public sharing is allowed based on settings.
	 *
	 * @return bool True if public sharing is allowed, false otherwise.
	 */
	public static function is_public_sharing_allowed() {
		return 'on' === self::get_settings_value('uncanny-allow-public-transcript-access', __CLASS__);
	}

	/**
	 * Validates the share token and retrieves the user ID if valid.
	 *
	 * @return int|false The user ID if the share token is valid, false otherwise.
	 */
	private static function get_valid_public_transcript_user() {
		$transcript_uuid = $_GET[self::$public_transcript_arg] ?? '';
		$transcript_uuid_params = self::get_public_sharing_params($transcript_uuid);
		if (false === $transcript_uuid_params) {
			return false;
		...
		// Reverse the MD5 hash to get the user ID's md5
		$uid_md5 = strrev($transcript_uuid_params[2]);

		global $wpdb;
		$table_name = $wpdb->prefix . 'users';
		
		// Query the database to find the user ID based on the MD5 hash
		$query = $wpdb->prepare("SELECT ID FROM $table_name WHERE MD5(ID) = %s LIMIT 1", $uid_md5);
		$user_id = $wpdb->get_var($query);

		// Return false if no user ID is found
		if (!$user_id) {
			return false;
		}

		// Validate the salted UUID hash
		$salted_uuid_hash = md5(self::$transcript_salt . $user_id);
		if ($transcript_uuid_params[1] !== $salted_uuid_hash) {
			return false;
		}
	...
	/**
	 * Generates the hash value for the user.
	 *
	 * @param \WP_User $user The user object.
	 * @return string The generated hash value.
	 */
	private static function get_hash_value($user) {
		return md5(self::$transcript_salt) . md5(self::$transcript_salt . $user->ID) . strrev(md5($user->ID));
	}
