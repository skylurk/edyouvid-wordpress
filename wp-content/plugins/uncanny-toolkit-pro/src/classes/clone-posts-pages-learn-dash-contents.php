...
class ClonePostsPagesLearnDashContents extends toolkit\Config implements toolkit\RequiredFunctions {
	...
	public static function add_clone_to_action_rows( $actions, $post ) {
		...
		if ( 'sfwd-quiz' === $post->post_type ) {
			$action = '?action=clone_learndash_quiz_contents&post=' . $post->ID . '&wpnonce=' . wp_create_nonce( __CLASS__ );
		} else {
			$action = '?action=clone_post_contents&post=' . $post->ID . '&wpnonce=' . wp_create_nonce( __CLASS__ );
		}
		$post_type_object = get_post_type_object( $post->post_type );
	...
	public static function uncanny_clone_quiz() {
		...
		if ( ! current_user_can( 'edit_published_posts' ) ) {
			wp_die( esc_attr__( 'You are not authorized to perform this action.', 'uncanny-pro-toolkit' ) );
		}

		if ( ! filter_has_var( INPUT_GET, 'wpnonce' ) || ! wp_verify_nonce( filter_input( INPUT_GET, 'wpnonce' ), __CLASS__ ) ) {
			wp_die( esc_attr__( 'You are not authorized to perform this action.', 'uncanny-pro-toolkit' ) );
		}

		if ( ! ( filter_has_var( INPUT_GET, 'post' ) || filter_has_var( INPUT_POST, 'post' ) || ( filter_has_var( INPUT_REQUEST, 'action' ) && 'clone_learndash_quiz_contents' === filter_input( INPUT_REQUEST, 'action' ) ) ) ) {
			wp_die( esc_attr__( 'No quiz to duplicate has been supplied!', 'uncanny-pro-toolkit' ) );
		}

		// Get the original post
		$id   = ( filter_has_var( INPUT_GET, 'post' ) ? filter_input( INPUT_GET, 'post' ) : filter_input( INPUT_POST, 'post' ) );
	...
	public static function uncanny_clone_contents() {
		...
		if ( isset( $post ) && $post != null ) {
			...
			if ( filter_has_var( INPUT_GET, 'return_to_post' ) ) {
	...
	public static function duplicate_contents_create_duplicate( $post, $status = '', $parent_id = '' ) {
		...
		// Temporarily enable shared steps and get the original values.
		$original_shared_steps_values = self::maybe_temporarily_enable_shared_steps();
		...
		// Restore the original shared steps values if necessary.
		self::maybe_restore_shared_steps_settings( $original_shared_steps_values );
		return $new_post_id;
	}

	/**
	 * Temporarily enable shared steps and return the original values to restore after the duplication.
	 * 
	 * This is a workaround to get around 2 bugs/issues : 
	 * 1. When duplicating a course, if the shared steps are not enabled, the original course content is removed.
	 * 2. Nested URLs permalink setting gets auto enabled when shared steps is enabled.
	 * 
	 * @return mixed - Array with original values or false if nothing to do.
	 */
	private static function maybe_temporarily_enable_shared_steps() {

		// Get current shared steps value
		$settings     = get_option( 'learndash_settings_courses_management_display', array() );
		$shared_steps = isset( $settings['course_builder_shared_steps'] ) ? $settings['course_builder_shared_steps'] : '';
		
		// Get current nested URLs value
		$permalinks  = get_option( 'learndash_settings_permalinks', array() );
		$nested_urls = isset( $permalinks['nested_urls'] ) ? $permalinks['nested_urls'] : '';
		
		// Only proceed if shared steps is not already enabled
		if ( 'yes' === $shared_steps ) {
			return false;
		}
		...
		// Temporarily enable shared steps
		$settings['course_builder_shared_steps'] = 'yes';
		update_option( 'learndash_settings_courses_management_display', $settings );
	...
	/**
	 * Restore original settings after duplication
	 * 
	 * @param array $original_values The original values from temporarily_enable_shared_steps()
	 * 
	 * @return void
	 */
	private static function maybe_restore_shared_steps_settings( $original_values ) {
		// Bail nothing to do.
		if ( ! is_array( $original_values ) ) {
			return;
		}

		// Restore the nested URLs setting if it was disabled.
		if ( 'yes' !== $original_values['nested_urls'] ) {
			// Set the required nonces for LearnDash settings
			$_POST['learndash_settings_permalinks']['nonce'] = wp_create_nonce( 'learndash_permalinks_nonce' );
			$permalinks = get_option( 'learndash_settings_permalinks', array() );
			// Restore the original nested URLs setting.
			$permalinks['nested_urls'] = $original_values['nested_urls'];
			update_option( 'learndash_settings_permalinks', $permalinks );
			// Clean up the POST data
			unset( $_POST['learndash_settings_permalinks']['nonce'] );
		}
		
		// Reset the shared steps setting.
		$settings = get_option( 'learndash_settings_courses_management_display', array() );
		// Restore the original shared steps setting.
		$settings['course_builder_shared_steps'] = $original_values['shared_steps'];
		update_option( 'learndash_settings_courses_management_display', $settings );
		
		// Clean up the POST data
		unset( $_POST['learndash_settings_courses_management_display_nonce'] );
	}
}
