...
class pseudo_performance_rest_call {
	...
	private function load_wp_minimally() {
		...
		// FlyWheel -- Load wp-load.php from a specific folder.
		if( file_exists( $_SERVER['DOCUMENT_ROOT'].'/.wordpress/wp-load.php' ) ){
	...
	private function define_constants(){
		 ...
		 * @since 3.5.5
		 */
		if ( ! defined( 'COOKIEHASH' ) && function_exists( 'get_option' ) ) {
	...
	private function run_uo_timer(){
		...
		if( ! isset($_GET['course_id'], $_GET['post_id'] ) ) {
			$return_object['success']             = false;
			$return_object['message']             = 'Invalid request.';
			$return_object['fields']['course_id'] = isset($_GET['course_id']) ? sanitize_text_field( $_GET['course_id'] ) : '';
			$return_object['fields']['post_id']   = isset($_GET['post_id']) ? sanitize_text_field( $_GET['post_id'] ) : '';
		...
		if ( ! $current_user ) {
			...
			$return_object['fields']['course_id'] = sanitize_text_field( $_GET['course_id'] );
			$return_object['fields']['post_id']   = sanitize_text_field( $_GET['post_id'] );
		...
		// validate inputs
		$course_ID = absint( sanitize_text_field( $_GET['course_id'] ));
		$post_ID   = absint( sanitize_text_field( $_GET['post_id'] ));
	...
	private function wp_validate_auth_cookie( $cookie = '', $scheme = 'logged_in' ) {
		...
		$sql  = "SELECT * FROM $wpdb->users WHERE user_login = %s";
		$q    = $wpdb->prepare( $sql, $username );
		$user = $wpdb->get_row( $q );
