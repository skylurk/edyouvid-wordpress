...
if ( ( class_exists( 'LearnDash_Settings_Metabox' ) ) && ( ! class_exists( 'LearnDash_Settings_Metabox_Topic_Group_Drip_Settings' ) ) ) {

	/**
	 * Class LearnDash_Settings_Metabox_Topic_Group_Drip_Settings
	 */
	class LearnDash_Settings_Metabox_Topic_Group_Drip_Settings extends \LearnDash_Settings_Metabox {
		...
		public function __construct() {
			...
			// translators: placeholder: topics.
				esc_html_x( 'Controls the timing and way %s can be accessed.', 'placeholder: topics', 'learndash' ),
		...
		public function load_settings_fields() {
			...
			if ( ! isset( $_GET['course_id'] ) ) {
				...
				$topic_courses = learndash_get_courses_for_step( $post->ID );
				if ( 'yes' === LearnDash_Settings_Section::get_section_setting( 'LearnDash_Settings_Courses_Builder', 'shared_steps' )
					 && (
						 ( isset( $topic_courses['primary'] ) && ! empty( $topic_courses['primary'] ) )
						 || ( isset( $topic_courses['primary'] ) && ! empty( $topic_courses['secondary'] ) )
					 )
				) {
					echo sprintf(
					...
					$topics_course_id = learndash_get_course_id( $post );

					if ( ! empty( $topics_course_id ) ) {
						$item_url = get_edit_post_link( $post->ID );
						$item_url = add_query_arg( 'course_id', $topics_course_id, $item_url );
						$item_url = add_query_arg( 'currentTab', 'uo-toolkit-topics', $item_url );
			...
			$note   = sprintf(
				_x( 'Note: A drip date set for a %1$s/Group persists across all %2$s. You cannot set different drip dates for the same %1$s/Group in different %2$s.', '1 Topic and 2 Course label', 'uncanny-pro-toolkit' ),
				LearnDash_Custom_Label::get_label( 'topic' ),
			...
			<script>
				function uoAddSwitcher() {
					jQuery('<div style="clear:both;padding: 15px 0;"><?php echo esc_html( $note ); ?></div>').insertBefore("#uo-ld-group-drip-topic");
					jQuery('<div style="clear:both; font-weight:bold; padding: 15px 0; text-align:right;"><?php echo $reload ?></div>').insertAfter("#uo-ld-group-drip-topic");
					jQuery('<div style="display:inline;"><?php echo $this->get_course_switch_html( $_GET["course_id"] ); ?></div>').appendTo("#uo-ld-group-drip-topic_filter");
					jQuery(".dataTables_filter label").addClass("pull-right");
				};
			</script>
			<table id="uo-ld-group-drip-topic" class="striped celled table" style="width:100%">
				<thead>
				<tr>
				...
				foreach ( $groups as $group ) {
					if ( $group && is_object( $group ) ) {
						...
						$date = get_post_meta( $post->ID, stripslashes( '\uncanny_pro_toolkit\UncannyDripTopicsByGroup' ) . '-' . $group->ID, true );
						// Add tha ( date ) after group name on selection if exists
						$u_date = '';
						if ( $date ) {
							if ( is_array( $date ) ) {
								$date   = \uncanny_pro_toolkit\UncannyDripTopicsByGroup::reformat_date( $date );
								$u_date = $date;
								$date   = learndash_adjust_date_time_display( $date );
							}

							if ( \uncanny_pro_toolkit\UncannyDripTopicsByGroup::is_timestamp( $date ) ) {
								$u_date = $date;
								$date   = \uncanny_pro_toolkit\UncannyDripTopicsByGroup::adjust_for_timezone_difference( $date );
							}
						}
						$date = absint( $date );
						?>
						<tr class="uo-ld-group-drip-topic-row" data-group="<?php echo $group->ID ?>"
						...
						>
							...
							<td>
								...
								<div class="ld_date_selector">
									<span
										class="screen-reader-text"><?php esc_attr_e( 'Month', 'uncanny-pro-toolkit' ); ?></span>
									...
									<span
										class="screen-reader-text"><?php esc_attr_e( 'Day', 'uncanny-pro-toolkit' ); ?></span>
									<input type="number" placeholder="DD" min="1" max="31"
										   class="ultp-gutenberg-field-text ld_date_jj"
										   name="day" value="<?php echo ! empty( $date ) ? date( 'd', $date ) : '' ?>"
										   size="2" maxlength="2"
										   autocomplete="off">,

									<span
										class="screen-reader-text"><?php esc_attr_e( 'Year', 'uncanny-pro-toolkit' ); ?></span>
									<input type="number" placeholder="YYYY"
										   min="<?php echo date( 'Y' ) - 3 ?>" max="<?php echo date( 'Y' ) + 15 ?>"
										   class="ultp-gutenberg-field-text ld_date_aa"
										   name="year" value="<?php echo ! empty( $date ) ? date( 'Y', $date ) : '' ?>"
										   size="4" maxlength="4"
										   step="1" autocomplete="off">
									@
									<span
										class="screen-reader-text"><?php esc_attr_e( 'Hour', 'uncanny-pro-toolkit' ); ?></span>
									<input type="number" min="0" max="23" placeholder="HH"
										   class="ultp-gutenberg-field-text ld_date_hh"
										   name="hour" value="<?php echo ! empty( $date ) ? date( 'H', $date ) : '' ?>"
										   size="2" maxlength="2"
										   autocomplete="off">:

									<span
										class="screen-reader-text"><?php esc_attr_e( 'Minute', 'uncanny-pro-toolkit' ); ?></span>
									<input type="number" min="0" max="59" placeholder="MN"
										   class="ultp-gutenberg-field-text ld_date_mn"
										   name="minute"
										   value="<?php echo ! empty( $date ) ? date( 'i', $date ) : '' ?>" size="2"
										   maxlength="2"
										   autocomplete="off">
								</div>
							</td>
							...
							<td class="uo-ld-group-drip-topic-row__actions">
								                                <button type="button"
                                        class="uo-ld-group-drip-topic__action uo-ld-group-drip-topic__action--save uncannyowl-btn uncannyowl-btn--primary">
									<?php esc_attr_e( 'Save date', 'uncanny-pro-toolkit' ); ?>
                                </button>

                                <button type="button"
                                        class="uo-ld-group-drip-topic__action uo-ld-group-drip-topic__action--remove uncannyowl-btn uncannyowl-btn--secondary">
	...
	add_filter(
		...
		function ( $metaboxes = array() ) {
			if ( ( ! isset( $metaboxes['LearnDash_Settings_Metabox_Topic_Group_Drip_Settings'] ) ) ) {
				$metaboxes['LearnDash_Settings_Metabox_Topic_Group_Drip_Settings'] = \LearnDash_Settings_Metabox_Topic_Group_Drip_Settings::add_metabox_instance();
			}
