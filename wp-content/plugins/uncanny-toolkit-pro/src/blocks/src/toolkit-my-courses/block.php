...
function render_uo_course_dashboard( $attributes ) {
	...
	} else if( is_numeric( $attributes['category'] ) ){
		// Lets find slug by ID
		$categoryTaxonomy = get_term_by('id', (int) $attributes['category'], 'category');
		if ($categoryTaxonomy) {
			$attributes['category'] = $categoryTaxonomy->slug;
		}
	}

	if ( empty( $attributes['ldCategory'] ) ){
		$attributes['ldCategory'] = 'all';
	} else if( is_numeric( $attributes['ldCategory'] ) ){
		// Lets find slug by ID
		$ldCategoryTaxonomy = get_term_by('id', (int) $attributes['ldCategory'], 'ld_course_category');
		if ($ldCategoryTaxonomy) {
			$attributes['ldCategory'] = $ldCategoryTaxonomy->slug;
		}
	}
	...
	$no_courses_message  = '';
	if ( isset( $attributes['no_courses_message'] ) ){
		$no_courses_message = $attributes['no_courses_message'];
	}
