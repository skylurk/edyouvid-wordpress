...
class Shortcode {
	 ...
	 * @var string[]
	 */
	public static $glightbox_transitions = array(
		'Fade' => 'fade',
		'Zoom' => 'zoom',
		'None' => 'none',
	);
	...
	function __construct() {
		 ...
		 * @since 3.6
		*/
		add_action( 'learndash-course-before', [ $this, 'single_step_course_override'], 99, 3 );
	}
	...
	public function wrap_complete_button( $button, $post ) {
		...
		if ( is_a( $post, 'WP_Post' ) &&
			...
			$post_option = ( is_object( $post ) ) ? \UCTINCAN\Admin\Metabox::get_meta_values( $post->ID ) : [];
			...
			$case = strtolower( $post_option['restrict-mark-complete'] );
			switch ( $case ) {
	...
	public function shortcode( $atts ) {
		...
		switch ( $embed_type ) {
			...
			case 'lightbox' :
				...
				if ( ! empty( (int) $global_lightbox ) ) {
					$width = $default_options['width'] . $default_options['width_type'];
					$height = $default_options['height'] . $default_options['height_type'];
				} else {
	...
	private function return_lightbox( $atts ) {
		...
		$width      = ( $width ) ? ' data-width="' . esc_attr($width) . '"' : '';
		$height     = ( $height ) ? ' data-height="' . esc_attr($height) . '"' : '';
		$transition = ( $transition ) ? ' data-effect="' . esc_attr($transition) . '"' : '';


		$this->enqueue_glightbox();

		return sprintf( '<a class="glightbox" href="%s" %s %s %s %s %s>%s</a>', $href, $title, $scrolling, $width, $height, $transition, $text );
	}
	...
	static public function generate_shortcode( $data ) {
		...
		// Modifying Name : TODO
		$data['title'] = Module_CRUD::change_name_from_id( $data['id'], $data['title'] );
		...
		switch ( $embed_type ) {
			...
			case 'lightbox' :
				...
				$global_lightbox = ! empty( $data['global_lightbox'] ) ? (int) $data['global_lightbox'] : '';
				...
				return sprintf( $shortcode . ' button="%s"%s%s%s%s%s%s%s%s%s href="%s" global_lightbox="%s"]', $data['lightbox_button'], $lightbox_title_text, $link_text, $btn_src, $scrollbar, $width, $height, $slider, $theme, $transition, $url, $global_lightbox );
				break;
	 ...
	 * @since 3.6
	 */
	public function single_step_course_override( $post_id, $course_id, $user_id ) {
		global $post;
