...
class Scorm extends \TINCANNYSNC\FileSystem\absModule {
	...
	public function __construct( $item_id ) {
		...
		if ( ! defined( 'UO_REPORTING_SCORM_DRIVER_JS_FILE_ASYNC' ) ) {
			define( 'UO_REPORTING_SCORM_DRIVER_JS_FILE_ASYNC', plugins_url('/src/assets/dist/scripts/scormdriver.js', UO_REPORTING_FILE) );
		}

		if ( ! defined( 'UO_REPORTING_SCORM_DRIVER_JS_FILE_SYNC' ) ) {
			define( 'UO_REPORTING_SCORM_DRIVER_JS_FILE_SYNC', plugins_url('/src/assets/dist/scripts/scormdriver-sync.js', UO_REPORTING_FILE) );
		}
	}
	...
	protected function get_registering_url() {
		...
		if ( file_exists( $imsmanifest_file ) ) {
			...
			$scorm_driver_js_url = UO_REPORTING_SCORM_DRIVER_JS_FILE_ASYNC;
			$scorm_driver_mode = (string) \TINCANNYSNC\Admin\Options::get_option( 'tinCanScormDriverCompatibility', 'async' );
			if( 'sync' === $scorm_driver_mode ) {
				$scorm_driver_js_url = UO_REPORTING_SCORM_DRIVER_JS_FILE_SYNC;
			}
