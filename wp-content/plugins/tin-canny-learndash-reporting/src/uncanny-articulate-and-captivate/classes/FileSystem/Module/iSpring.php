...
class iSpring extends \TINCANNYSNC\FileSystem\absModule {
	...
	protected function add_tincan_support() {
		...
		// Check for FlipBook/PDF Viewer module - these use SCORM API, not TinCan
		$is_flipbook = strstr( $ispring_html, '<!--content book -->' ) || 
		               strstr( $ispring_html, 'iSpring.pdfviewer.LMS' ) ||
		               strstr( $ispring_html, 'PdfViewer.open' );
		
		if ( $is_flipbook ) {
			// Preserve original lms.js but fix iSpring export bugs
			return true;
		}
		...
		// tincan.xml exists and this is not version 11 - stop.
		if ( file_exists( $target . '/tincan.xml' ) && ! $is_version_11 ) {
			return true;
		}
		
		// Copy lms.js - iSpring-lms.js || iSpring-lms-quiz.js || iSpring11-lms.js
		unlink( $file );
