...
if ( ! class_exists( 'LoginPress_Set_User' ) ) :
	...
	class LoginPress_Set_User {
		...
		public function __construct() {
			add_action( 'init', array( $this, 'login_user_with_autologin_code' ), 11 );
						...
						if ( 1 === count( $user_ids ) ) {
							...
							if ( ( isset( $user_meta_status ) && 'inactive' === $user_meta_status ) && ( isset( $loginpress_options['enable_user_verification'] ) && 'on' === $loginpress_options['enable_user_verification'] ) ) {
								...
								exit;
									...
									exit;
							} elseif ( 'unchecked' === $user_meta['expire'] || empty( $user_meta['expire'] ) ) {
		 ...
		 *
		 * @param WP_Error $errors The login error messages.
		 * @since 1.0.0
		 * @version 6.1.0
		 * @return WP_Error The login errors with custom error message.
		 */
		public function loginpress_autologin_custom_error( $errors ) {
			...
			if ( 'user_inactive' === $error ) {
				$error = apply_filters( 'loginpress_autologin_inactive_error', __( 'User Verification Is Required', 'loginpress-pro' ) );
				$errors->add( 'access', sanitize_text_field( $error ) ); // @codingStandardsIgnoreLine.

			} elseif ( 'user_disabled' === $error ) {
				$error = apply_filters( 'loginpress_autologin_disable_error', __( 'User is disabled by Admin', 'loginpress-pro' ) );
				$errors->add( 'access', sanitize_text_field( $error ) ); // @codingStandardsIgnoreLine.

			} elseif ( 'expired' === $error ) {
				$error = apply_filters( 'loginpress_autologin_expired_error', __( 'This Auto login link is Expired', 'loginpress-pro' ) );
				$errors->add( 'access', sanitize_text_field( $error ) ); // @codingStandardsIgnoreLine.

			} elseif ( 'invalid_login_code' === $error ) {
				$error = apply_filters( 'loginpress_autologin_invalid_login_code', __( 'This code is invalid', 'loginpress-pro' ) );
				$errors->add( 'access', sanitize_text_field( $error ) ); // @codingStandardsIgnoreLine.
		...
		/**
		 * Generates string of the GET variable including '?' separator from the URL.
		 *
		...
		/**
		 * Convert a GET variable array into GET-request parameter list.
		 *
