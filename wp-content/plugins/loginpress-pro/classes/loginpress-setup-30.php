...
if ( ! class_exists( 'LoginPress_Pro_Setup_30' ) ) :
	...
	class LoginPress_Pro_Setup_30 {
		 ...
		 * Addon Slugs.
		 *
		 * @var array Array of addon slugs.
		 */
		 ...
		 * Class Constructor.
		 *
		 * @param bool $show_notice Whether to show or hide the notice of setup.
		 * @return void
		 * @since 3.0.0
		 * @version 6.1.0
		 */
		...
		public function loginpress_free_update_plugin() {
			...
			if ( ! current_user_can( 'update_plugins' ) ) {
				$status['errorMessage'] = __( 'Sorry, you are not allowed to update plugins for this site.', 'loginpress-pro' );
			...
			if ( empty( $_POST['plugin'] ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error(
					array(
						...
						'errorMessage' => __( 'No plugin specified.', 'loginpress-pro' ),
					)
				);
			}
			...
			if ( false !== $result ) {
				...
				$status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'loginpress-pro' );
			...
			$status['errorMessage'] = __( 'Plugin update failed.', 'loginpress-pro' );
			wp_send_json_error( $status );
			wp_die();
		}
		/**
		...
		public function active_pro_addon( $plugin_updated ) {
			...
			if ( 'loginpress/loginpress.php' === $plugin_updated ) {
				...
				activate_plugin( 'loginpress/loginpress.php' );
			} elseif ( is_plugin_active( $plugin_updated ) ) {
		...
		public function setup_notice() {
			if ( function_exists( 'get_current_screen' ) ) {
				...
				if ( LOGINPRESS_VERSION <= '3.0.5' && $this->show_notice ) {
					...
					printf(
					/* translators: 1: opening div tag, 2: closing div tag, 3: opening p tag, 4: closing p tag, 5: opening a tag, 6: closing a tag */
						esc_html__( '%1$s%3$sPlease complete the %5$ssetup%6$s for LoginPress 3.0%4$s%2$s', 'loginpress-pro' ),
						'<div class="notice notice-error">',
						'</div>',
						'<p>',
						'</p>',
						'<a href="' . esc_url( admin_url() . 'admin.php?page=loginpress-setup-30' ) . '">',
						'</a>'
					);
				}
			}
		...
		public function loginpress_pro_30_setup_page() {
			add_submenu_page( 'loginpress-settings', __( 'Setup 3.0', 'loginpress-pro' ), __( 'Setup 3.0', 'loginpress-pro' ), 'manage_options', 'loginpress-setup-30', array( $this, 'loginpress_pro_setup_30' ) );
		}
		...
		public function loginpress_pro_setup_30() {
			...
			<div class="loginpress-pro-setup-30-wrapper">
				...
				?>
				<div class="loginpress-pro-30-wrapper loginpress-pro-30-update-addons-wrapper"<?php echo $display_element; // phpcs:ignore ?> >
						<div class="loginpress-pro-30-inner-wrapper">
				...
				<div class="loginpress-pro-30-wrapper loginpress-pro-30-finish-wrapper" <?php echo esc_attr( $display_element ); ?> >
						<div class="loginpress-pro-30-inner-wrapper">
		...
		public function loginpress_update_autologin_meta() {
			...
			$user_query = new WP_User_Query(
				array(
					'meta_key' => 'loginpress_autologin_code', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
				)
			);
