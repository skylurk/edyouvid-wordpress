...
class LoginPress_AddOn_Updater {
	...
	public function plugins_api_filter( $_data, $_action = '', $_args = null ) {
		...
		if ( ( 'plugin_information' !== $_action ) || ! isset( $_args->slug ) || ( $this->slug !== $_args->slug ) ) {
			return $_data;
		}
	...
	public function show_changelog() {
		...
		$addon_id = isset( $_REQUEST['addon_id'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['addon_id'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		if ( $response && isset( $response->sections['changelog'] ) ) {
			echo '<div style="background:#fff;padding:10px;height:100%;">' . wp_kses_post( $response->sections['changelog'] ) . '</div>'; // @codingStandardsIgnoreLine.
		}
		exit;
	}
}
